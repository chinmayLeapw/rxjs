...
describe('fromEvent', function () {
        ...
        rxTestScheduler.run(function (_a) {
            ...
            var target = {
                addEventListener: function (eventType, listener) {
                    rxjs_1.timer(delay1, delay2).pipe(operators_1.mapTo('ev'), operators_1.take(2), operators_1.concat(rxjs_1.NEVER)).subscribe(listener);
                },
    ...
    it('should pass through options to addEventListener and removeEventListener', function () {
        ...
        var subscription = rxjs_1.fromEvent(obj, 'click', expectedOptions).subscribe(function () {
        });
        subscription.unsubscribe();
    ...
    it('should pass through events that occur and use the selector if provided', function (done) {
        ...
        rxjs_1.fromEvent(obj, 'click', selector)
            .pipe(operators_1.take(1))
            .subscribe({
            next: function (e) {
                chai_1.expect(e).to.equal('test!');
    ...
    it('should handle adding events to an arraylike of targets', function () {
        ...
        subscription.unsubscribe();
        chai_1.expect(nodeList[0]._removeEventListenerArgs).to.deep.equal(nodeList[0]._addEventListenerArgs);
        chai_1.expect(nodeList[1]._removeEventListenerArgs).to.deep.equal(nodeList[1]._addEventListenerArgs);
    });
});
