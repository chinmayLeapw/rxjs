...
describe('animationFrames', function () {
    ...
    it('should animate', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '   ---a---b---c';
            var subs = '       ^----------!';
            var result = mapped.pipe(operators_1.mergeMapTo(rxjs_1.animationFrames()));
    ...
    it('should use any passed timestampProvider', function () {
        ...
        testScheduler.run(function (_a) {
            var animate = _a.animate, cold = _a.cold, expectObservable = _a.expectObservable;
            animate('            ---x---x---x');
            var mapped = cold('-m          ');
            var expected = '   ---a---b---c';
            var subs = '       ^----------!';
            var result = mapped.pipe(operators_1.mergeMapTo(rxjs_1.animationFrames(timestampProvider)));
            expectObservable(result, subs).toBe(expected, {
    ...
    it('should compose with take', function () {
        testScheduler.run(function (_a) {
            var animate = _a.animate, cold = _a.cold, expectObservable = _a.expectObservable, time = _a.time;
            var requestSpy = sinon.spy(animationFrameProvider_1.animationFrameProvider.delegate, 'requestAnimationFrame');
            var cancelSpy = sinon.spy(animationFrameProvider_1.animationFrameProvider.delegate, 'cancelAnimationFrame');
            animate('            ---x---x---x');
            var mapped = cold('-m          ');
            var tm = time('    -|          ');
            var ta = time('    ---|        ');
            var tb = time('    -------|    ');
            var expected = '   ---a---b    ';
            var result = mapped.pipe(operators_1.mergeMapTo(rxjs_1.animationFrames().pipe(operators_1.take(2))));
    ...
    it('should compose with takeUntil', function () {
        testScheduler.run(function (_a) {
            var animate = _a.animate, cold = _a.cold, expectObservable = _a.expectObservable, hot = _a.hot, time = _a.time;
            var requestSpy = sinon.spy(animationFrameProvider_1.animationFrameProvider.delegate, 'requestAnimationFrame');
            var cancelSpy = sinon.spy(animationFrameProvider_1.animationFrameProvider.delegate, 'cancelAnimationFrame');
            animate('            ---x---x---x');
            var mapped = cold('-m          ');
            ...
            var signal = hot(' ^--------s--');
            var expected = '   ---a---b    ';
            var result = mapped.pipe(operators_1.mergeMapTo(rxjs_1.animationFrames().pipe(operators_1.takeUntil(signal))));
            expectObservable(result).toBe(expected, {
                a: { elapsed: ta - tm, timestamp: ta },
                b: { elapsed: tb - tm, timestamp: tb },
            });
