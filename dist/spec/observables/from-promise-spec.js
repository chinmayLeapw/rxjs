...
describe('from (fromPromise)', function () {
    it('should emit one value from a resolved promise', function (done) {
        ...
        rxjs_1.from(promise)
            .subscribe({ next: function (x) { chai_1.expect(x).to.equal(42); }, error: function (x) {
    ...
    it('should accept already-resolved Promise', function (done) {
        ...
        promise.then(function (x) {
            ...
            rxjs_1.from(promise)
                .subscribe({ next: function (y) { chai_1.expect(y).to.equal(42); }, error: function (x) {
                    done(new Error('should not be called'));
                }, complete: function () {
                    done();
                } });
    ...
    it('should accept PromiseLike object for interoperability', function (done) {
        ...
        rxjs_1.from(promise)
            .subscribe({ next: function (x) { chai_1.expect(x).to.equal(42); }, error: function () {
                done(new Error('should not be called'));
            }, complete: function () {
                done();
            } });
    ...
    it('should raise error from a rejected promise on a separate scheduler', function (done) {
        var promise = Promise.reject('bad');
        rxjs_1.from(promise, rxjs_1.asapScheduler)
            .subscribe({ next: function (x) { done(new Error('should not be called')); }, error: function (e) {
                chai_1.expect(e).to.equal('bad');
                done();
            }, complete: function () {
                done(new Error('should not be called'));
