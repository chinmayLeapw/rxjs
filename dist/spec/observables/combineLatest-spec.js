...
describe('static combineLatest', function () {
    ...
    it('should combine an immediately-scheduled source with an immediately-scheduled second', function (done) {
        ...
        var actual = [];
        rxjs_1.combineLatest(a, b, queueScheduler).subscribe({ next: function (vals) {
                actual.push(vals);
            }, error: function () {
    ...
    it('should accept a dictionary of observables', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var secondSource = hot('--d--e--f--g--|');
            var expected = '        ----uv--wx-y--z----|';
            var combined = rxjs_1.combineLatest({ a: firstSource, b: secondSource }).pipe(operators_1.map(function (_a) {
                var a = _a.a, b = _a.b;
                return '' + a + b;
            }));
            expectObservable(combined).toBe(expected, { u: 'ad', v: 'ae', w: 'af', x: 'bf', y: 'bg', z: 'cg' });
