...
describe('zip', function () {
    ...
    it('should combine a source with a second', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^                ';
            var expected = '---x---y---z     ';
            expectObservable(rxjs_1.zip(a, b)).toBe(expected, { x: ['1', '4'], y: ['2', '5'], z: ['3', '6'] });
    ...
    it('should zip the provided observables', function (done) {
        ...
        var i = 0;
        rxjs_1.zip(rxjs_1.from(['a', 'b', 'c']), rxjs_1.from([1, 2, 3]), function (a, b) { return a + b; }).subscribe({
            next: function (x) {
    ...
    it('should end once one observable completes and its buffer is empty', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(rxjs_1.zip(e1, e2, e3)).toBe(expected, values);
    ...
    describe('with iterables', function () {
        it('should zip them with values', function () {
            rxTestScheduler.run(function (_a) {
                ...
                expectObservable(rxjs_1.zip(e1, myIterator)).toBe(expected, values);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
        it('should work with empty observable and empty iterable', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var b = [];
                var expected = '|';
                expectObservable(rxjs_1.zip(a, b)).toBe(expected);
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
        ...
        it('should work with non-empty observable and non-empty iterable', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var b = [2];
                var expected = '-----(x|)';
                expectObservable(rxjs_1.zip(a, b)).toBe(expected, { x: ['1', 2] });
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
        ...
        it('should work with non-empty observable and non-empty iterable selector that throws', function () {
            rxTestScheduler.run(function (_a) {
                ...
                expectObservable(rxjs_1.zip(a, b, selector)).toBe(expected, { x: '14' }, new Error('too bad'));
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
    });
    it('should combine two observables and selector', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^                ';
            var expected = '---x---y---z     ';
            expectObservable(rxjs_1.zip(a, b, function (e1, e2) { return e1 + e2; })).toBe(expected, { x: '14', y: '25', z: '36' });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with n-ary symmetric', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var c = hot('---1-^---3---6-|  ');
            var expected = '  ----x---y-|  ';
            expectObservable(rxjs_1.zip(a, b, c)).toBe(expected, { x: ['1', '2', '3'], y: ['4', '5', '6'] });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    ...
    it('should work with some data asymmetric 1', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(rxjs_1.zip(a, b, function (r1, r2) { return r1 + r2; })).toBe(expected, { a: '12', b: '34', c: '56', d: '78', e: '90' });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with some data asymmetric 2', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '     ^-----------------!    ';
            var expected = '  ---a--b--c--d--e--|    ';
            expectObservable(rxjs_1.zip(a, b, function (r1, r2) { return r1 + r2; })).toBe(expected, { a: '21', b: '43', c: '65', d: '87', e: '09' });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    ...
    it('should work with error and error', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^-----!    ';
            var expected = '------#    ';
            expectObservable(rxjs_1.zip(a, b)).toBe(expected, null, 'too bad');
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with two sources that eventually raise errors', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^-------!    ';
            var expected = '-----x--#    ';
            expectObservable(rxjs_1.zip(a, b)).toBe(expected, { x: [1, 2] }, 'too bad');
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with two sources that eventually raise errors (swapped)', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^-------!    ';
            var expected = '-----x--#    ';
            expectObservable(rxjs_1.zip(a, b)).toBe(expected, { x: [2, 1] }, 'too bad');
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    ...
    it('should be able to zip all iterables', function () {
        ...
        chai_1.expect(results).to.deep.equal([['a', '1', 'x'], ['b', '2', 'y'], ['c', '3', 'z'], 'complete']);
    });
