...
describe('from', function () {
    ...
    it('should create an observable from an array', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var delayTime = time('--|');
            var expected = 'x-y-(z|)';
            var e1 = rxjs_1.from([10, 20, 30]).pipe(operators_1.concatMap(function (x, i) { return rxjs_1.of(x).pipe(operators_1.delay(i === 0 ? 0 : delayTime)); }));
        ...
        source.subscribe({
            ...
            complete: function () {
                ...
                setTimeout(function () {
                    ...
                    chai_1.expect(results).to.deep.equal([0, 1, 2, 'done', 'finalized generator']);
                    done();
                });
            },
        });
    ...
    it('should finalize an AsyncGenerator on error', function (done) {
        ...
        var source = rxjs_1.from(gen()).pipe(operators_1.tap({
            next: function (value) {
                if (value === 2) {
        ...
        source.subscribe({
            ...
            error: function () {
                ...
                setTimeout(function () {
                    chai_1.expect(sideEffects).to.deep.equal([0, 1, 2]);
                    chai_1.expect(results).to.deep.equal([0, 1, 'in error', 'finalized generator']);
                    done();
                });
            },
        });
    ...
    it('should finalize an AsyncGenerator on unsubscribe', function (done) {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        ...
                        case 6:
                            results.push('finalized generator');
                            chai_1.expect(sideEffects).to.deep.equal([0, 1, 2]);
                            chai_1.expect(results).to.deep.equal([0, 1, 'finalized generator']);
                            done();
                            return [7];
                        case 7: return [2];
                    }
        ...
        subscription = source.subscribe(function (value) { return results.push(value); });
    });
    ...
    var sources = [
        { name: 'observable', createValue: function () { return rxjs_1.of('x'); } },
        { name: 'observable-like', createValue: function () { return fakervable('x'); } },
        { name: 'observable-like-array', createValue: function () { return fakeArrayObservable('x'); } },
        { name: 'array', createValue: function () { return ['x']; } },
        { name: 'promise', createValue: function () { return Promise.resolve('x'); } },
        { name: 'iterator', createValue: function () { return fakerator('x'); } },
        { name: 'array-like', createValue: function () {
                var _a;
        ...
        { name: 'string', createValue: function () { return 'x'; } },
        { name: 'arguments', createValue: function () { return getArguments('x'); } },
    ];
    if (Symbol && Symbol.asyncIterator) {
        ...
        sources.push({
            name: 'async-iterator',
            createValue: function () { return fakeAsyncIterator_1('x'); },
        });
    }
    ...
    it('should appropriately handle errors from an iterator', function () {
        ...
        rxjs_1.from(erroringIterator).subscribe({
            ...
            error: function (err) { return results.push(err.message); },
        });
