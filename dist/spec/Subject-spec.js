...
describe('Subject', function () {
    ...
    it('should allow next with undefined or any when created with no type', function (done) {
        ...
        subject.subscribe({ next: function (x) {
    ...
    it('should handle subscribers that arrive and leave at different times, ' + 'subject does not complete', function () {
        ...
        subject.next(3);
        subject.next(4);
        var subscription1 = subject.subscribe({ next: function (x) {
                results1.push(x);
            }, error: function (e) {
                results1.push('E');
            }, complete: function () {
        ...
        subject.next(5);
        var subscription2 = subject.subscribe({ next: function (x) {
                results2.push(x);
            }, error: function (e) {
                results2.push('E');
            }, complete: function () {
        ...
        subject.next(9);
        subject.next(10);
        var subscription3 = subject.subscribe({ next: function (x) {
                results3.push(x);
            }, error: function (e) {
                results3.push('E');
            }, complete: function () {
    ...
    it('should not next after completed', function () {
        var subject = new rxjs_1.Subject();
        var results = [];
        subject.subscribe({ next: function (x) { return results.push(x); }, complete: function () { return results.push('C'); } });
        subject.next('a');
        subject.complete();
        subject.next('b');
    ...
    it('should not next after error', function () {
        ...
        var subject = new rxjs_1.Subject();
        var results = [];
        subject.subscribe({ next: function (x) { return results.push(x); }, error: function (err) { return results.push(err); } });
        subject.next('a');
        subject.error(error);
        subject.next('b');
    ...
    describe('asObservable', function () {
        ...
        it('should work with inherited subject', function () {
            ...
            subject.complete();
            var observable = subject.asObservable();
            observable.subscribe({ next: function (x) { return results.push(x); }, complete: function () { return results.push('done'); } });
...
describe('useDeprecatedSynchronousErrorHandling', function () {
    ...
    it('should throw an error when nexting with a flattened, erroring inner observable', function () {
        var subject = new rxjs_1.Subject();
        subject.pipe(operators_1.mergeMap(function () { return rxjs_1.throwError(function () { return new Error('bad'); }); })).subscribe();
        chai_1.expect(function () {
            subject.next('wee');
        }).to.throw(Error, 'bad');
    });
    it('should throw an error when nexting with a flattened, erroring inner observable with more than one operator', function () {
        var subject = new rxjs_1.Subject();
        subject.pipe(operators_1.mergeMap(function () { return rxjs_1.throwError(function () { return new Error('bad'); }); }), operators_1.map(function (x) { return x; })).subscribe();
        chai_1.expect(function () {
            subject.next('wee');
        }).to.throw(Error, 'bad');
    });
    it('should throw an error when notifying an error with catchError returning an erroring inner observable', function () {
        var subject = new rxjs_1.Subject();
        subject.pipe(operators_1.catchError(function () { return rxjs_1.throwError(function () { return new Error('bad'); }); })).subscribe();
        chai_1.expect(function () {
            subject.error('wee');
        }).to.throw(Error, 'bad');
    });
    it('should throw an error when nexting with an operator that errors synchronously', function () {
        var subject = new rxjs_1.Subject();
        subject.pipe(operators_1.mergeMap(function () {
            throw new Error('lol');
        })).subscribe();
        chai_1.expect(function () {
            subject.next('wee');
    ...
    it('should throw an error when notifying an error with a catchError that errors synchronously', function () {
        var subject = new rxjs_1.Subject();
        subject.pipe(operators_1.catchError(function () {
            throw new Error('lol');
        })).subscribe();
        chai_1.expect(function () {
            subject.error('wee');
    ...
    it('should not throw on second error passed, even after having been operated on', function () {
        var subject = new rxjs_1.Subject();
        subject.pipe(operators_1.mergeMap(function (x) { return [x]; })).subscribe();
        chai_1.expect(function () {
            subject.error(new Error('one'));
        }).to.throw(Error, 'one');
        chai_1.expect(function () {
    ...
    it('deep rethrowing 1', function () {
        ...
        subject2.subscribe();
        subject1.subscribe({
            next: function () { return subject2.error(new Error('hahaha')); }
        });
        chai_1.expect(function () {
            subject1.next('test');
