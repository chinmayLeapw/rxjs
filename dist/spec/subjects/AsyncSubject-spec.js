...
describe('AsyncSubject', function () {
    ...
    it('should allow reentrant subscriptions', function () {
        ...
        subject.subscribe({
            next: function (value) {
                subject.subscribe({
                    next: function (value) { return results.push('inner: ' + (value + value)); },
                    complete: function () { return results.push('inner: done'); },
                });
                results.push('outer: ' + value);
            },
            complete: function () { return results.push('outer: done'); },
        });
        subject.next(1);
        chai_1.expect(results).to.deep.equal([]);
        subject.complete();
        chai_1.expect(results).to.deep.equal(['inner: 2', 'inner: done', 'outer: 1', 'outer: done']);
    });
});
