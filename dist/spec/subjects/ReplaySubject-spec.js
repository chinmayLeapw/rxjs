...
describe('ReplaySubject', function () {
    ...
    describe('with bufferSize=2', function () {
        it('should replay 2 previous values when subscribed', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var subscriber1 = hot('------(a|)                         ').pipe(operators_1.mergeMapTo(replaySubject));
                var unsub1 = '         ---------------------!             ';
                var expected1 = '      ------(23)4------5-6--             ';
                var subscriber2 = hot('------------(b|)                   ').pipe(operators_1.mergeMapTo(replaySubject));
                var unsub2 = '         -------------------------!         ';
                var expected2 = '      ------------(34)-5-6---7--         ';
                var subscriber3 = hot('---------------------------(c|)    ').pipe(operators_1.mergeMapTo(replaySubject));
                var expected3 = '      ---------------------------(78)9--|';
                expectObservable(hot(sourceTemplate).pipe(operators_1.tap({ next: feedNextIntoSubject, error: feedErrorIntoSubject, complete: feedCompleteIntoSubject }))).toBe(sourceTemplate);
    ...
    describe('with windowTime=4', function () {
        it('should replay previous values since 4 time units ago when subscribed', function () {
            rxTestScheduler.run(function (_a) {
                var hot = _a.hot, expectObservable = _a.expectObservable;
                var replaySubject = new rxjs_1.ReplaySubject(Infinity, 4, rxTestScheduler);
                function feedNextIntoSubject(x) {
                    replaySubject.next(x);
                }
                function feedErrorIntoSubject(err) {
    ...
    it('should be an Observer which can be given to Observable.subscribe', function () {
        ...
        var results = [];
        subject.subscribe({ next: function (x) { return results.push(x); }, complete: function () { return results.push('done'); } });
    ...
    it('should not buffer nexted values after error', function () {
        ...
        subject.subscribe({
            next: function (value) { return results.push(value); },
            error: function () { return results.push('E'); },
        });
