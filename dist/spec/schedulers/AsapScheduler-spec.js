...
describe('Scheduler.asap', function () {
    ...
    it('should act like the async scheduler if delay > 0', function () {
        testScheduler.run(function (_a) {
            var cold = _a.cold, expectObservable = _a.expectObservable, time = _a.time;
            var a = cold('  a            ');
            var ta = time(' ----|        ');
            var b = cold('  b            ');
            var tb = time(' --------|    ');
            var expected = '----a---b----';
            var result = rxjs_1.merge(a.pipe(operators_1.delay(ta, asap)), b.pipe(operators_1.delay(tb, asap)));
            expectObservable(result).toBe(expected);
        });
    });
    it('should cancel asap actions when delay > 0', function () {
        testScheduler.run(function (_a) {
            var cold = _a.cold, expectObservable = _a.expectObservable, flush = _a.flush, time = _a.time;
            var sandbox = sinon.createSandbox();
            var setImmediateSpy = sandbox.spy(immediateProvider_1.immediateProvider, 'setImmediate');
            var setSpy = sandbox.spy(intervalProvider_1.intervalProvider, 'setInterval');
            var clearSpy = sandbox.spy(intervalProvider_1.intervalProvider, 'clearInterval');
            var a = cold('  a            ');
            var ta = time(' ----|        ');
            var subs = '    ^-!          ';
            var expected = '-------------';
    ...
    it('should reuse the interval for recursively scheduled actions with the same delay', function () {
        ...
        var stubSetInterval = sandbox.stub(global, 'setInterval').callThrough();
        var period = 50;
    ...
    it('should properly cancel an unnecessary flush', function (done) {
        var sandbox = sinon.createSandbox();
        var clearImmediateStub = sandbox.stub(immediateProvider_1.immediateProvider, 'clearImmediate').callThrough();
        var a;
        var b;
        var c;
        a = rxjs_1.asapScheduler.schedule(function () {
