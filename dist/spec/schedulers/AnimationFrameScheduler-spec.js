...
describe('Scheduler.animationFrame', function () {
    ...
    it('should act like the async scheduler if delay > 0', function () {
        testScheduler.run(function (_a) {
            ...
            var tb = time(' --------|    ');
            var expected = '----a---b----';
            var result = rxjs_1.merge(a.pipe(operators_1.delay(ta, animationFrame)), b.pipe(operators_1.delay(tb, animationFrame)));
            expectObservable(result).toBe(expected);
        });
    });
    it('should cancel animationFrame actions when delay > 0', function () {
        testScheduler.run(function (_a) {
            var animate = _a.animate, cold = _a.cold, expectObservable = _a.expectObservable, flush = _a.flush, time = _a.time;
            var requestSpy = sinon.spy(animationFrameProvider_1.animationFrameProvider, 'requestAnimationFrame');
            var setSpy = sinon.spy(intervalProvider_1.intervalProvider, 'setInterval');
            var clearSpy = sinon.spy(intervalProvider_1.intervalProvider, 'clearInterval');
            animate('         ----------x--');
            var a = cold('  a            ');
            var ta = time(' ----|        ');
            var subs = '    ^-!          ';
    ...
    it('should execute actions scheduled when flushing in a subsequent flush', function (done) {
        ...
        var stubFlush = (sandbox.stub(rxjs_1.animationFrameScheduler, 'flush')).callThrough();
        var a;
        var b;
        var c;
    ...
    it('should properly cancel an unnecessary flush', function (done) {
        var sandbox = sinon.createSandbox();
        var cancelAnimationFrameStub = sandbox.stub(animationFrameProvider_1.animationFrameProvider, 'cancelAnimationFrame').callThrough();
        var a;
        var b;
        var c;
        a = rxjs_1.animationFrameScheduler.schedule(function () {
