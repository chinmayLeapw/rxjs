...
describe('timestamp', function () {
    ...
    it('should record stamp if source emit elements', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.timestamp(rxTestScheduler))).toBe(expected, expectedValue);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should completes without record stamp if source does not emits', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var e1subs = '  ^--------!';
            var expected = '---------|';
            expectObservable(e1.pipe(operators_1.timestamp(rxTestScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.timestamp(), operators_1.take(3)).subscribe(function () {
        });
