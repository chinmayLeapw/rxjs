...
describe('sampleTime', function () {
    ...
    it('should get samples on a delay', function () {
        rxTest.run(function (_a) {
            ...
            var expected = '   -------c-------------e------h-|';
            var period = time('-------|                       ');
            expectObservable(e1.pipe(operators_1.sampleTime(period, rxTest))).toBe(expected);
    ...
    it('should allow unsubscribing explicitly and early', function () {
        rxTest.run(function (_a) {
            ...
            var expected = '    -----------c-----            ';
            var period = time(' -----------|                 ');
            expectObservable(e1.pipe(operators_1.sampleTime(period, rxTest)), unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        rxTest.run(function (_a) {
            ...
            var expected = '    -----------c-----            ';
            var unsub = '       ----------------!            ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.sampleTime(period, rxTest), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
    ...
    it('should not complete if source does not complete', function () {
        rxTest.run(function (_a) {
            ...
            var period = time('-----|  ');
            var e1unsbs = '    -------!';
            expectObservable(e1.pipe(operators_1.sampleTime(period, rxTest)), e1unsbs).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
});
