...
describe('repeatWhen operator', function () {
    ...
    it('should repeat when notified via returned notifier on complete', function (done) {
        ...
        try {
            rxjs_1.of(1, 2)
                ...
                return notifications.pipe(operators_1.map(function (x) {
                    if (retried) {
    ...
    it('should not repeat when applying an empty notifier', function (done) {
        ...
        rxjs_1.of(1, 2)
            .pipe(operators_1.map(function (n) {
            return n;
        }), operators_1.repeatWhen(function (notifications) { return rxjs_1.EMPTY; }))
            .subscribe({
            next: function (n) {
    ...
    it('should not error when applying an empty synchronous notifier', function () {
        ...
        rxjs_1.Observable.prototype.subscribe = function () {
            ...
            var _a = __read(args, 1), subscriber = _a[0];
            if (!(subscriber instanceof rxjs_1.Subscriber)) {
                subscriber = new (Subscriber_1.SafeSubscriber.bind.apply(Subscriber_1.SafeSubscriber, __spreadArray([void 0], __read(args))))();
            }
        ...
        rxjs_1.of(1, 2)
            .pipe(operators_1.repeatWhen(function (notifications) { return rxjs_1.EMPTY; }))
            .subscribe({ error: function (err) { return errors.push(err); } });
    ...
    it('should not error when applying a non-empty synchronous notifier', function () {
        ...
        rxjs_1.of(1, 2)
            .pipe(operators_1.repeatWhen(function (notifications) { return rxjs_1.of(1); }))
            .subscribe({ error: function (err) { return errors.push(err); } });
        rxjs_1.Observable.prototype.subscribe = originalSubscribe;
        chai_1.expect(errors).to.deep.equal([]);
    });
    ...
    it('should propagate error thrown from notifierSelector function', function () {
        testScheduler.run(function (_a) {
            ...
            var subs = '       ^----------!';
            var expected = '   --a--b--c--#';
            var result = source.pipe(operators_1.repeatWhen((function () {
                throw 'bad!';
            })));
    ...
    it.skip('should handle a host source that completes via operator like take, and a hot notifier', function () {
        testScheduler.run(function (_a) {
            ...
            var nsubs = '       -------^-----------!';
            var expected = '    -1--2----------5---|';
            var result = source.pipe(operators_1.takeWhile(function (value) { return value !== '3'; }), operators_1.repeatWhen(function () { return notifier; }));
            expectObservable(result).toBe(expected);
    ...
    it('should handle a source with eventual error using a dynamic notifier selector which eventually completes', function () {
        testScheduler.run(function (_a) {
            ...
            var invoked = 0;
            var result = source.pipe(operators_1.repeatWhen(function (notifications) {
                return notifications.pipe(operators_1.map(function () { return 'x'; }), operators_1.takeUntil(notifications.pipe(operators_1.mergeMap(function () {
                    if (++invoked < 3) {
                        return rxjs_1.EMPTY;
                    }
                    else {
    ...
    it('should always finalize before starting the next cycle, even when synchronous', function () {
        ...
        var subscription = source.pipe(operators_1.repeatWhen(function (completions$) { return completions$.pipe(operators_1.takeWhile(function (_, i) { return i < 3; })); })).subscribe({
        ...
        chai_1.expect(results).to.deep.equal([1, 2, 'finalizer', 1, 2, 'finalizer', 1, 2, 'finalizer', 1, 2, 'complete', 'finalizer']);
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.repeatWhen(function () { return rxjs_1.of(0); }), operators_1.take(3))
            .subscribe(function () {
        });
