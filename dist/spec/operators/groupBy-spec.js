...
describe('groupBy operator', function () {
    ...
    it('should group numbers by odd/even', function () {
        testScheduler.run(function (_a) {
            ...
            var source = e1.pipe(operators_1.groupBy(function (val) { return parseInt(val) % 2; }));
    ...
    it('should group values', function (done) {
        ...
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.groupBy(function (x) { return x % 2; }))
            .subscribe({
            next: function (g) {
    ...
    it('should group values with a duration selector', function () {
        ...
        var resultingGroups = [];
        rxjs_1.of(1, 2, 3, 4, 5, 6)
            .pipe(operators_1.groupBy(function (x) { return x % 2; }, {
            duration: function (g) { return g.pipe(operators_1.skip(1)); },
        }))
    ...
    it('should handle an empty Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  (^!)';
            var expected = '|   ';
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }));
    ...
    it('should allow an inner to be unsubscribed early but other inners continue', function () {
        testScheduler.run(function (_a) {
            ...
            var z = '            ------------f-------------|';
            var expectedGroups = {
                w: testing_1.TestScheduler.parseMarbles(w, values, undefined, undefined, true),
                x: testing_1.TestScheduler.parseMarbles(x, values, undefined, undefined, true),
                y: testing_1.TestScheduler.parseMarbles(y, values, undefined, undefined, true),
                z: testing_1.TestScheduler.parseMarbles(z, values, undefined, undefined, true),
            };
            var fooUnsubscriptionFrame = testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubw, true).unsubscribedFrame;
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }), operators_1.map(function (group) {
                var arr = [];
                var subscription = group.pipe(phonyMarbelize(testScheduler)).subscribe(function (value) {
                    arr.push(value);
                });
                if (group.key === 'foo') {
    ...
    it('should allow inners to be unsubscribed early at different times', function () {
        testScheduler.run(function (_a) {
            ...
            var unsubscriptionFrames = {
                foo: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubw, true).unsubscribedFrame,
                bar: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubx, true).unsubscribedFrame,
                baz: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsuby, true).unsubscribedFrame,
                qux: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubz, true).unsubscribedFrame,
            };
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }), operators_1.map(function (group) {
                var arr = [];
                var subscription = group.pipe(phonyMarbelize(testScheduler)).subscribe(function (value) {
    ...
    it('should allow subscribing late to an inner Observable, outer completes', function () {
        testScheduler.run(function (_a) {
            var hot = _a.hot, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions, time = _a.time;
            var values = {
                a: '  foo',
                b: ' FoO ',
                d: 'foO ',
            ...
            var expected = '        ----------------------------|';
            e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); })).subscribe(function (group) {
                testScheduler.schedule(function () {
    ...
    it('should allow subscribing late to an inner Observable, outer throws', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '         ----------------------------#';
            e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); })).subscribe({
                next: function (group) {
                    testScheduler.schedule(function () {
                        expectObservable(group).toBe(expected);
                    }, subsDuration);
    ...
    it('should allow using a keySelector, elementSelector, and durationSelector', function () {
        testScheduler.run(function (_a) {
            ...
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, function (val) { return reverseString(val); }, function (group) { return group.pipe(operators_1.skip(2)); }));
            expectObservable(source).toBe(expected, expectedValues);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should allow using a keySelector, elementSelector, and durationSelector that throws', function () {
        testScheduler.run(function (_a) {
            ...
            var z = cold('                         i-----l-|', reversedValues);
            var expectedValues = { v: v, w: w, x: x, y: y, z: z };
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, function (val) { return reverseString(val); }, function (group) {
                return group.pipe(operators_1.skip(2), operators_1.map(function () {
                    throw 'error';
                }));
            }));
            expectObservable(source).toBe(expected, expectedValues);
    ...
    it('should allow using a keySelector and a durationSelector, outer throws', function () {
        testScheduler.run(function (_a) {
            ...
            var expectedValues = { v: v, w: w, x: x, y: y, z: z };
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, {
                duration: function (group) { return group.pipe(operators_1.skip(2)); },
            }));
            expectObservable(source).toBe(expected, expectedValues);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    ...
    it('should allow using a durationSelector, outer and all inners unsubscribed early', function () {
        testScheduler.run(function (_a) {
            ...
            var x = '            ----------e-';
            var expectedGroups = {
                v: testing_1.TestScheduler.parseMarbles(v, values, undefined, undefined, true),
                w: testing_1.TestScheduler.parseMarbles(w, values, undefined, undefined, true),
                x: testing_1.TestScheduler.parseMarbles(x, values, undefined, undefined, true),
            };
            var unsubscriptionFrame = testing_1.TestScheduler.parseMarblesAsSubscriptions(unsub, true).unsubscribedFrame;
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, {
                duration: function (group) { return group.pipe(operators_1.skip(2)); },
            }), operators_1.map(function (group) {
                var arr = [];
    ...
    it('should dispose a durationSelector after closing the group', function () {
        testScheduler.run(function (_a) {
            var cold = _a.cold, hot = _a.hot, expectSubscriptions = _a.expectSubscriptions;
            var obs = hot(' -0-1--------2-| ');
            var sub = '     ^--------------!';
            var unsubs = [
                '              -^--!',
            ...
            var unsubscribedFrame = testing_1.TestScheduler.parseMarblesAsSubscriptions(sub, true).unsubscribedFrame;
            obs
                .pipe(operators_1.groupBy(function (val) { return val; }, {
                duration: function (group) { return durations[Number(group.key)]; },
            }))
                .subscribe();
            testScheduler.schedule(function () {
    ...
    it('should allow an inner to be unsubscribed early but other inners continue, with durationSelector', function () {
        testScheduler.run(function (_a) {
            ...
            var z = '            ------------------i-----l-|';
            var expectedGroups = {
                v: testing_1.TestScheduler.parseMarbles(v, reversedValues, undefined, undefined, true),
                w: testing_1.TestScheduler.parseMarbles(w, reversedValues, undefined, undefined, true),
                x: testing_1.TestScheduler.parseMarbles(x, reversedValues, undefined, undefined, true),
                y: testing_1.TestScheduler.parseMarbles(y, reversedValues, undefined, undefined, true),
                z: testing_1.TestScheduler.parseMarbles(z, reversedValues, undefined, undefined, true),
            };
            var fooUnsubscriptionFrame = testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubv, true).unsubscribedFrame;
    ...
    it('should allow inners to be unsubscribed early at different times, with durationSelector', function () {
        testScheduler.run(function (_a) {
            ...
            var unsubscriptionFrames = {
                foo: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubv, true).unsubscribedFrame,
                bar: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubw, true).unsubscribedFrame,
                baz: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubx, true).unsubscribedFrame,
                qux: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsuby, true).unsubscribedFrame,
                foo2: testing_1.TestScheduler.parseMarblesAsSubscriptions(unsubz, true).unsubscribedFrame,
            };
            var hasUnsubscribed = {};
            var source = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, function (val) { return val; }, function (group) { return group.pipe(operators_1.skip(2)); }), operators_1.map(function (group) {
                ...
                var unsubscriptionFrame = hasUnsubscribed[group.key] ? unsubscriptionFrames[group.key + '2'] : unsubscriptionFrames[group.key];
                testScheduler.schedule(function () {
                    subscription.unsubscribe();
    ...
    it('should return inners that when subscribed late exhibit hot behavior', function () {
        testScheduler.run(function (_a) {
            ...
            var subscriptionFrames = {
                foo: testing_1.TestScheduler.parseMarblesAsSubscriptions(subv, true).subscribedFrame,
                bar: testing_1.TestScheduler.parseMarblesAsSubscriptions(subw, true).subscribedFrame,
                baz: testing_1.TestScheduler.parseMarblesAsSubscriptions(subx, true).subscribedFrame,
                qux: testing_1.TestScheduler.parseMarblesAsSubscriptions(suby, true).subscribedFrame,
            };
            var result = e1.pipe(operators_1.groupBy(function (val) { return val.toLowerCase().trim(); }, function (val) { return val; }), operators_1.map(function (group) {
                ...
                testScheduler.schedule(function () {
                    group.pipe(phonyMarbelize(testScheduler)).subscribe(function (value) {
    ...
    it('should return inner group that when subscribed late emits complete()', function () {
        testScheduler.run(function (_a) {
            ...
            var g = '            --------------------------------|';
            var expectedGroups = {
                g: testing_1.TestScheduler.parseMarbles(g, values, undefined, undefined, true),
            };
            var innerSubscriptionFrame = testing_1.TestScheduler.parseMarblesAsSubscriptions(innerSub, true).subscribedFrame;
    ...
    it('should not error for late subscribed inners if outer is unsubscribed before inners are subscribed', function () {
        testScheduler.run(function (_a) {
            ...
            var result = source.pipe(operators_1.groupBy(function (char) { return char; }), operators_1.tap({
                next: function (group) {
                    testScheduler.schedule(function () { return group.subscribe(subjects[group.key]); }, 1000);
                },
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.groupBy(function (value) { return value; }), operators_1.take(3))
            .subscribe(function () {
        });
...
function phonyMarbelize(testScheduler) {
    return function (source) {
        return source.pipe(operators_1.materialize(), operators_1.map(function (notification) {
            return {
                frame: testScheduler.frame,
                notification: NotificationFactories_1.createNotification(notification.kind, notification.value, notification.error),
            };
        }));
    };
}
