...
describe('last', function () {
    ...
    it('should error on nothing sent but completed', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^----!';
            var expected = '   -----#';
            expectObservable(e1.pipe(operators_1.last())).toBe(expected, null, new rxjs_1.EmptyError());
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should allow undefined as a default value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------------!   ';
            var expected = '--------------(U|)';
            expectObservable(e1.pipe(operators_1.last(function (value) { return value === 'b'; }, undefined))).toBe(expected, { U: undefined });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should return last element matches with predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------------!   ';
            var expected = '--------------(b|)';
            expectObservable(e1.pipe(operators_1.last(function (value) { return value === 'b'; }))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should support type guards without breaking previous behavior', function () {
        {
            ...
            rxjs_1.of(foo)
                .pipe(operators_1.last(function (foo) { return foo.bar === 'name'; }))
                .subscribe(function (x) { return x.baz; });
            rxjs_1.of(foo)
            ...
            rxjs_1.of(foobar)
                ...
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(foobar)
                .pipe(operators_1.last(function (foobar) { return foobar.bar === 'name'; }))
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(foobar)
            ...
            rxjs_1.of(barish)
                ...
                .subscribe(function (x) { return x.baz; });
            rxjs_1.of(barish)
                .pipe(operators_1.last(function (x) { return x.bar === 'quack'; }))
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(barish)
                .pipe(operators_1.last(isBar))
                .subscribe(function (x) { return x.bar; });
        }
        {
            ...
            xs.pipe(operators_1.last(isString)).subscribe(function (s) { return s.length; });
            xs.pipe(operators_1.last(function (x) { return typeof x === 'string'; })).subscribe(function (x) { return x; });
        }
    });
});
