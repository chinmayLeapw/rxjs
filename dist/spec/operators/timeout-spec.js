...
describe('timeout operator', function () {
    ...
    it('should timeout after a specified timeout period', function () {
        rxTestScheduler.run(function (_a) {
            var cold = _a.cold, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions, time = _a.time;
            var e1 = cold(' -------a--b--|');
            var t = time('  -----|        ');
            var e1subs = '  ^----!        ';
            var expected = '-----#        ';
    ...
    it('should not timeout if source completes within absolute timeout period', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.timeout(timeoutValue, rxTestScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should not timeout if source emits within timeout period', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----------------!';
            var expected = '--a--b--c--d--e--|';
            expectObservable(e1.pipe(operators_1.timeout(t, rxTestScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var expected = '--a--b--c--        ';
            var unsub = '   ----------!        ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.timeout(t, rxTestScheduler), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    describe('config', function () {
        it('should timeout after a specified timeout period', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var e1subs = '  ^----!        ';
                var expected = '-----#        ';
                var result = e1.pipe(operators_1.timeout({
                    each: t,
                }));
                expectObservable(result).toBe(expected, null, defaultTimeoutError);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
        ...
        it('should not timeout if source completes within absolute timeout period', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var e1subs = '  ^----------------!';
                var expected = '--a--b--c--d--e--|';
                expectObservable(e1.pipe(operators_1.timeout({ first: new Date(t) }))).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
        it('should not timeout if source emits within timeout period', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var e1subs = '  ^----------------!';
                var expected = '--a--b--c--d--e--|';
                expectObservable(e1.pipe(operators_1.timeout({ each: t }))).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
        it('should allow unsubscribing explicitly and early', function () {
        ...
        it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var expected = '--a--b--c--        ';
                var unsub = '   ----------!        ';
                var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.timeout({ each: t }), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
                expectObservable(result, unsub).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
        ...
        it('should timeout for long delays if first is specified as Date AND each is specified', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var e1subs = '    ^--------!';
                var expected = '  ---a-----#';
                var result = e1.pipe(operators_1.timeout({ first: new Date(first), each: each }));
                expectObservable(result).toBe(expected, undefined, defaultTimeoutError);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
    ...
    describe('using with', function () {
        it('should timeout after a specified period then subscribe to the passed observable', function () {
            rxTestScheduler.run(function (_a) {
                var hot = _a.hot, cold = _a.cold, time = _a.time, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions;
                var source = cold('  -------a--b--|');
                var sourceSubs = '   ^----!        ';
                var t = time('       -----|');
                ...
                var innerSubs = '    -----^-----!  ';
                var expected = '     -----x-y-z-|  ';
                var result = source.pipe(operators_1.timeout({
                    each: t,
                    with: function () { return inner; },
                }));
        ...
        it('should allow unsubscribing explicitly and early', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var expected = '    ---a---b----x--    ';
                var unsub = '       --------------!    ';
                var result = source.pipe(operators_1.timeout({ each: t, with: function () { return inner; } }));
                expectObservable(result, unsub).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                expectSubscriptions(inner.subscriptions).toBe(innerSubs);
            });
        ...
        it('should not break unsubscription chain when unsubscribed explicitly', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var expected = '    ---a---b----x--    ';
                var unsub = '       --------------!    ';
                var result = source.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.timeout({ each: t, with: function () { return inner; } }), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
                expectObservable(result, unsub).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                expectSubscriptions(inner.subscriptions).toBe(innerSubs);
            });
        ...
        it('should not timeout if source completes within specified Date', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var inner = cold('--x--|            ');
                var expected = '  --a--b--c--d--e--|   ';
                var result = source.pipe(operators_1.timeout({ first: new Date(t), with: function () { return inner; } }));
                expectObservable(result).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                expectSubscriptions(inner.subscriptions).toBe([]);
            });
        ...
        it('should not timeout if source emits synchronously when subscribed', function () {
            rxTestScheduler.run(function (_a) {
                var expectObservable = _a.expectObservable, time = _a.time;
                var source = rxjs_1.of('a').pipe(operators_1.concatWith(rxjs_1.NEVER));
                var t = time('  ---|');
                var expected = 'a---';
                expectObservable(source.pipe(operators_1.timeout({ first: new Date(t) }))).toBe(expected);
            });
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.timeout(0), operators_1.take(3)).subscribe(function () {
        });
