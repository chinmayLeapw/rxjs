...
describe('zipAll operator', function () {
    ...
    it('should combine paired events from two observables', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var expected = ' -----------------A----B-|';
            var result = outer.pipe(operators_1.zipAll(function (a, b) { return a + b; }));
    ...
    it('should combine two observables', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected, values);
    ...
    it('should take all observables from the source and zip them', function (done) {
        ...
        var i = 0;
        rxjs_1.of(rxjs_1.of('a', 'b', 'c'), rxjs_1.of(1, 2, 3))
    ...
    it('should end once one observable completes and its buffer is empty', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(rxjs_1.of(e1, e2, e3).pipe(operators_1.zipAll())).toBe(expected, values);
    ...
    describe('with iterables', function () {
        it('should zip them with values', function () {
            rxTestScheduler.run(function (_a) {
                ...
                expectObservable(rxjs_1.of(e1, myIterator).pipe(operators_1.zipAll())).toBe(expected, values);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
            });
        });
        it('should complete instantly with never observable and empty iterable', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var b = [];
                var expected = '|';
                expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected);
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
        ...
        it('should work with non-empty observable and non-empty iterable', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var b = [2];
                var expected = '-----(x|)';
                expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected, { x: ['1', 2] });
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
        ...
        it('should work with non-empty many observable and non-empty many iterable', function () {
            rxTestScheduler.run(function (_a) {
                ...
                var b = [4, 5, 6];
                var expected = '---x--y--(z|)';
                expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected, { x: ['1', 4], y: ['2', 5], z: ['3', 6] });
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
        it('should work with non-empty observable and non-empty iterable selector that throws', function () {
            rxTestScheduler.run(function (_a) {
                ...
                expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll(selector))).toBe(expected, { x: '14' }, new Error('too bad'));
                expectSubscriptions(a.subscriptions).toBe(asubs);
            });
        });
    });
    it('should combine two observables and selector', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^';
            var expected = '---x---y---z';
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll(function (e1, e2) { return e1 + e2; }))).toBe(expected, { x: '14', y: '25', z: '36' });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with n-ary symmetric', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var c = hot('   ---1-^---3---6-|  ');
            var expected = '     ----x---y-|  ';
            expectObservable(rxjs_1.of(a, b, c).pipe(operators_1.zipAll())).toBe(expected, { x: ['1', '2', '3'], y: ['4', '5', '6'] });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    ...
    it('should work with some data asymmetric 1', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll(function (r1, r2) { return r1 + r2; }))).toBe(expected, { a: '12', b: '34', c: '56', d: '78', e: '90' });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with some data asymmetric 2', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '     ^-----------------!    ';
            var expected = '  ---a--b--c--d--e--|    ';
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll(function (r1, r2) { return r1 + r2; }))).toBe(expected, { a: '21', b: '43', c: '65', d: '87', e: '09' });
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    ...
    it('should raise error if inner observable raises error', function () {
        rxTestScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.zipAll())).toBe(expected, expectedValues);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(z.subscriptions).toBe(zsubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should work with error and error', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^-----!';
            var expected = '------#';
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected, null, 'too bad');
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with two sources that eventually raise errors', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^-------!';
            var expected = '-----x--#';
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected, { x: [1, 2] }, 'too bad');
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    it('should work with two sources that eventually raise errors (swapped)', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var bsubs = '   ^-------!';
            var expected = '-----x--#';
            expectObservable(rxjs_1.of(a, b).pipe(operators_1.zipAll())).toBe(expected, { x: [2, 1] }, 'too bad');
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
        });
    });
    ...
    it('should combine two immediately-scheduled observables', function (done) {
        rxTestScheduler.run(function () {
            ...
            var i = 0;
            var result = rxjs_1.scheduled([a, b], rxjs_1.queueScheduler).pipe(operators_1.zipAll());
            result.subscribe({
                next: function (vals) {
