...
describe('isEmpty', function () {
    ...
    it('should return true if source is empty', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----!   ';
            var expected = '-----(T|)';
            expectObservable(e1.pipe(operators_1.isEmpty())).toBe(expected, { T: true });
    ...
    it('should return false if source emits element', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^--!   ';
            var expected = '   ---(F|)';
            expectObservable(e1.pipe(operators_1.isEmpty())).toBe(expected, { F: false });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-------           ';
            var unsub = '   ------!           ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.isEmpty(), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
