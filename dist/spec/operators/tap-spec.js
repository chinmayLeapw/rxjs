...
describe('tap', function () {
    ...
    it('should raise error if error handler raises error', function () {
        rxjs_1.throwError(function () { return 'ops'; })
            .pipe(operators_1.tap({
            error: function () {
                throw new Error('bad');
            },
    ...
    describe('lifecycle handlers', function () {
        it('should support an unsubscribe event that fires before finalize', function () {
            ...
            var subscription = subject
                .pipe(operators_1.tap({
                subscribe: function () { return results.push('subscribe'); },
                next: function (value) { return results.push("next " + value); },
                error: function (err) { return results.push("error: " + err.message); },
                complete: function () { return results.push('complete'); },
                unsubscribe: function () { return results.push('unsubscribe'); },
                finalize: function () { return results.push('finalize'); },
            }))
                .subscribe();
            subject.next(1);
            subject.next(2);
            ...
            chai_1.expect(results).to.deep.equal(['subscribe', 'next 1', 'next 2', 'unsubscribe', 'finalize']);
        });
        it('should not call unsubscribe if source completes', function () {
            ...
            subject.complete();
            subscription.unsubscribe();
            chai_1.expect(results).to.deep.equal(['subscribe', 'next 1', 'next 2', 'complete', 'finalize']);
        });
        it('should not call unsubscribe if source errors', function () {
            ...
            subscription.unsubscribe();
            chai_1.expect(results).to.deep.equal(['subscribe', 'next 1', 'next 2', 'error: bad', 'finalize']);
        });
    });
});
