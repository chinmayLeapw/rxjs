...
describe('retry', function () {
    ...
    it('should retry a number of times, then call next handler without error, then retry and complete', function (done) {
        ...
        var errors = 0;
        var retries = 2;
        rxjs_1.defer(function () { return rxjs_1.range(0, 4 - index); })
            .pipe(operators_1.mergeMap(function () {
            index++;
            if (index === 1 || index === 3) {
                errors++;
                return rxjs_1.throwError(function () { return 'bad'; });
            }
            else {
                return rxjs_1.of(42);
            }
    ...
    it('should always finalize before starting the next cycle, even when synchronous', function () {
        ...
        chai_1.expect(results).to.deep.equal([1, 2, 'finalizer', 1, 2, 'finalizer', 1, 2, 'finalizer', 1, 2, 'bad', 'finalizer']);
    });
    ...
    it('should retry until successful completion', function (done) {
        ...
        new rxjs_1.Observable(function (observer) {
            ...
            errors = 0;
            return x;
        }), operators_1.retry(), operators_1.take(retries))
            .subscribe({
            next: function (x) {
                chai_1.expect(x).to.equal(42);
            },
    ...
    it('should retry a synchronous source (multicasted and refCounted) multiple times', function (done) {
        ...
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.concat(rxjs_1.throwError(function () { return 'bad!'; })), operators_1.multicast(function () { return new rxjs_1.Subject(); }), operators_1.refCount(), operators_1.retry(4))
            .subscribe({
            next: function (x) {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.retry(1), operators_1.take(3)).subscribe(function () {
        });
    ...
    describe('with delay config', function () {
        describe('of a number', function () {
            ...
            it('should act like a normal retry if delay is set to 0', function () {
                rxTest.run(function (_a) {
                    var cold = _a.cold, expectSubscriptions = _a.expectSubscriptions, expectObservable = _a.expectObservable;
                    var source = cold('---a---b---#');
                    var subs = [
                        '                  ^----------!',
                        '                  -----------^----------!',
        ...
        describe('of a function', function () {
            ...
            it('should delay the retry with a function that returns a hot observable', function () {
                rxTest.run(function (_a) {
                    var cold = _a.cold, hot = _a.hot, expectSubscriptions = _a.expectSubscriptions, expectObservable = _a.expectObservable;
                    var source = cold(' ---a---b---#');
                    var notifier = hot('--------------x----------------x----------------x------');
                    var subs = [
                        '                   ^----------!',
                        '                   --------------^----------!',
                        '                   -------------------------------^----------!',
                    ...
                    var expected = '    ---a---b---------a---b------------a---b------';
                    var result = source.pipe(operators_1.retry({
                        delay: function () { return notifier; },
                    }));
                    expectObservable(result, unsub).toBe(expected);
                    expectSubscriptions(source.subscriptions).toBe(subs);
            ...
            it('should complete if the notifier completes', function () {
                rxTest.run(function (_a) {
                    ...
                    var result = source.pipe(operators_1.retry({
                        delay: function (_err, retryCount) {
                            return retryCount <= 2 ? rxjs_1.timer(retryCount) : rxjs_1.EMPTY;
                        },
                    }));
                    expectObservable(result).toBe(expected);
                    expectSubscriptions(source.subscriptions).toBe(subs);
            ...
            it('should error if the notifier errors', function () {
                rxTest.run(function (_a) {
                    ...
                    var result = source.pipe(operators_1.retry({
                        delay: function (_err, retryCount) {
                            return retryCount <= 2 ? rxjs_1.timer(retryCount) : rxjs_1.throwError(function () { return new Error('blah'); });
                        },
                    }));
            ...
            it('should be usable for exponential backoff', function () {
                rxTest.run(function (_a) {
                    ...
                    var result = source.pipe(operators_1.retry({
                        count: 3,
                        delay: function (_err, retryCount) { return rxjs_1.timer(Math.pow(2, retryCount)); },
                    }));
                    expectObservable(result).toBe(expected);
                    expectSubscriptions(source.subscriptions).toBe(subs);
                });
