...
describe('toArray', function () {
    ...
    it('should reduce the values of an observable into an array', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var e1subs = '  ^--------!   ';
            var expected = '---------(w|)';
            expectObservable(e1.pipe(operators_1.toArray())).toBe(expected, { w: ['a', 'b'] });
    ...
    it('should be empty when source is empty', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var e1subs = '  (^!)';
            var expected = '(w|)';
            expectObservable(e1.pipe(operators_1.toArray())).toBe(expected, { w: [] });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should reduce the a single value of an observable into an array', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var e1subs = '   ^-----!  ';
            var expected = ' ------(w|)';
            expectObservable(e1.pipe(operators_1.toArray())).toBe(expected, { w: ['y'] });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should work with error', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var e1subs = '   ^--------!';
            var expected = ' ---------#';
            expectObservable(e1.pipe(operators_1.toArray())).toBe(expected, null, 'too bad');
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
