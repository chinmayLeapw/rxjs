...
describe('debounceTime', function () {
    ...
    it('should not break unsubscription chains when unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   -------!       ';
            var t = time('  --|');
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.debounceTime(t), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should unsubscribe from the scheduled debounce action when downstream unsubscribes', function () {
        var scheduler = new AnimationFrameScheduler_1.AnimationFrameScheduler(AnimationFrameAction_1.AnimationFrameAction);
        chai_1.expect(scheduler._scheduled).to.not.exist;
        chai_1.expect(scheduler.actions).to.be.empty;
        var subscription = rxjs_1.NEVER.pipe(operators_1.startWith(1), operators_1.debounceTime(0, scheduler)).subscribe();
