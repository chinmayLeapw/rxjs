...
describe('mergeScan', function () {
    ...
    it('should mergeScan things', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeScan(function (acc, x) { return rxjs_1.of(acc.concat(x)); }, []));
    ...
    it('should mergeScan values and be able to asynchronously project them', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeScan(function (acc, x) { return rxjs_1.of(acc.concat(x)).pipe(operators_1.delay(t)); }, []));
            expectObservable(result).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.mergeScan(function (acc, x) { return rxjs_1.of(acc.concat(x)).pipe(operators_1.delay(t)); }, []), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        rxjs_1.of(null)
            .pipe(operators_1.mergeScan(function () { return synchronousObservable; }, 0), operators_1.takeWhile(function (x) { return x != 2; }))
            .subscribe(function () {
        });
    ...
    it('should handle errors in the projection function', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeScan(function (acc, x) {
                if (x === 'd') {
                    throw new Error('bad!');
                }
    ...
    it('should propagate errors from the projected Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^--!';
            var expected = '   ---#';
            var result = e1.pipe(operators_1.mergeScan(function () { return rxjs_1.throwError(function () { return new Error('bad!'); }); }, []));
    ...
    it('should handle an empty projected Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^--------------------!';
            var expected = '   ---------------------|';
            var result = e1.pipe(operators_1.mergeScan(function () { return rxjs_1.EMPTY; }, []));
    ...
    it('should handle a never projected Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^--------------------!';
            var expected = '   ----------------------';
            var result = e1.pipe(operators_1.mergeScan(function () { return rxjs_1.NEVER; }, []));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should mergeScan projects cold Observable with single concurrency', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeScan(function (acc, x) { return inner[+x].pipe(operators_1.startWith(acc)); }, 'x', 1));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should not emit accumulator if inner completes without value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^--------------------!';
            var expected = '   ---------------------|';
            var result = e1.pipe(operators_1.mergeScan(function () { return rxjs_1.EMPTY; }, ['1']));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should not emit accumulator if inner completes without value after source completes', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '     ^--------!  ';
            var expected = '   -----------|';
            var result = e1.pipe(operators_1.mergeScan(function () { return x; }, '1'));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should mergeScan projects cold Observable with dual concurrency', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeScan(function (acc, x) { return inner[+x].pipe(operators_1.startWith(acc)); }, 'x', 2));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(inner[0].subscriptions).toBe(xsubs);
            expectSubscriptions(inner[1].subscriptions).toBe(ysubs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.mergeScan(function (acc, value) { return rxjs_1.of(value); }, 0), operators_1.take(3))
            .subscribe(function () {
        });
