...
describe('delay', function () {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '------a--   ';
            var unsub = '   --------!   ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.delay(t), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should unsubscribe scheduled actions after execution', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.repeatWhen(function (notifications) {
                ...
                next: function () {
                    var _a = __read(subscribeSpy.args, 1), _b = __read(_a[0], 1), subscriber = _b[0];
    ...
    it('should be possible to delay complete by composition', function () {
        testScheduler.run(function (_a) {
            ...
            var t = time('     --|       ');
            var expected = '-----a--b---|';
            var result = rxjs_1.concat(e1.pipe(operators_1.delay(t)), rxjs_1.of(undefined).pipe(operators_1.delay(t), operators_1.ignoreElements()));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
