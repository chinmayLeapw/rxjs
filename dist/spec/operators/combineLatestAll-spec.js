...
describe('combineLatestAll operator', function () {
    ...
    it('should combine events from two observables', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = ' -----------------A-B--C---|';
            var result = outer.pipe(operators_1.combineLatestAll(function (a, b) { return String(a) + String(b); }));
    ...
    it('should work with two nevers', function () {
        testScheduler.run(function (_a) {
            ...
            var e2subs = '  ^';
            var expected = '-';
            var result = rxjs_1.of(e1, e2).pipe(operators_1.combineLatestAll(function (x, y) { return x + y; }));
    ...
    it('should work with hot-single and hot-empty', function () {
        testScheduler.run(function (_a) {
            ...
            var e2subs = '   ^---!';
            var expected = ' ----|';
            var result = rxjs_1.of(e2, e1).pipe(operators_1.combineLatestAll(function (x, y) { return x + y; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
        });
    ...
    it('should not break unsubscription chains when unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '        ---------!    ';
            var values = { x: 'bf', y: 'cf', z: 'cg' };
            var result = rxjs_1.of(e1, e2).pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.combineLatestAll(function (x, y) { return x + y; }), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
        });
    ...
    it('should combine 3 observables', function () {
        testScheduler.run(function (_a) {
            ...
            var e3subs = '     ^---------!';
            var expected = '   -----wxyz-|';
            var result = rxjs_1.of(e1, e2, e3).pipe(operators_1.combineLatestAll(function (x, y, z) { return x + y + z; }));
    ...
    it('should handle throw after complete left', function () {
        testScheduler.run(function (_a) {
            ...
            var rightSubs = '     ^--------!';
            var expected = '      ---------#';
            var result = rxjs_1.of(left, right).pipe(operators_1.combineLatestAll(function (x, y) { return x + y; }));
    ...
    it('should handle selector throwing', function () {
        testScheduler.run(function (_a) {
            ...
            var e2subs = '     ^--!';
            var expected = '   ---#';
            var result = rxjs_1.of(e1, e2).pipe(operators_1.combineLatestAll(function (x, y) {
                throw 'ha ha ' + x + ', ' + y;
            }));
