...
describe('distinctUntilKeyChanged', function () {
    ...
    it('should distinguish between values', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------------------!';
            var expected = '--a--------b-----a--|';
            expectObservable(e1.pipe(operators_1.distinctUntilKeyChanged('val'))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not complete if source never completes', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '      ^';
            var expected = '    -';
            expectObservable(e1.pipe(operators_1.distinctUntilKeyChanged('val'))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should emit once if comparer returns true always regardless of source emits', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----------------!';
            var expected = '--a--------------|';
            expectObservable(e1.pipe(operators_1.distinctUntilKeyChanged('val', function () { return true; }))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should emit all if comparer returns false always regardless of source emits', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.distinctUntilKeyChanged('val', function () { return false; }))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should distinguish values by selector', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.distinctUntilKeyChanged('val', selector))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.map(function (value) { return ({ value: value }); }), operators_1.distinctUntilKeyChanged('value'), operators_1.take(3))
            .subscribe(function () {
        });
