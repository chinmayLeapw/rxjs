...
describe('publishLast operator', function () {
    ...
    it('should multicast the same values to multiple observers', function () {
        testScheduler.run(function (_a) {
            ...
            var sourceSubs = '     ^-----------!   ';
            var published = source.pipe(operators_1.publishLast());
            var subscriber1 = hot('a|              ').pipe(operators_1.mergeMapTo(published));
            var expected1 = '      ------------(4|)';
            var subscriber2 = hot('----b|          ').pipe(operators_1.mergeMapTo(published));
            var expected2 = '      ------------(4|)';
            var subscriber3 = hot('--------c|      ').pipe(operators_1.mergeMapTo(published));
            var expected3 = '      ------------(4|)';
    ...
    it('should not cast any values to multiple observers, when source is unsubscribed explicitly and early', function () {
        testScheduler.run(function (_a) {
            var cold = _a.cold, hot = _a.hot, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions;
            var source = cold('    -1-2-3----4-|');
            var sourceSubs = '     ^--------!   ';
    ...
    describe('with refCount()', function () {
        it('should connect when first subscriber subscribes', function () {
            testScheduler.run(function (_a) {
                ...
                var subscriber1 = hot('---a|              ').pipe(operators_1.mergeMapTo(replayed));
                var expected1 = '      ---------------(4|)';
                var subscriber2 = hot('-------b|          ').pipe(operators_1.mergeMapTo(replayed));
                var expected2 = '      ---------------(4|)';
                var subscriber3 = hot('-----------c|      ').pipe(operators_1.mergeMapTo(replayed));
                var expected3 = '      ---------------(4|)';
                expectObservable(subscriber1).toBe(expected1);
                expectObservable(subscriber2).toBe(expected2);
                expectObservable(subscriber3).toBe(expected3);
    ...
    it('should be referentially-transparent', function () {
        testScheduler.run(function (_a) {
            ...
            var source2Subs = ' ^----------!   ';
            var expected2 = '   -----------(0|)';
            var partialPipeLine = rxjs_1.pipe(operators_1.publishLast());
