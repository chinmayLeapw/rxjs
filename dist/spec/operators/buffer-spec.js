...
describe('Observable.prototype.buffer', function () {
    ...
    it('should work with non-empty and throw selector', function () {
        testScheduler.run(function (_a) {
            var hot = _a.hot, expectObservable = _a.expectObservable;
            var a = hot('---^--a--');
            var b = rxjs_1.throwError(function () { return new Error('too bad'); });
            var expected = '#';
    ...
    it('should work with throw and non-empty selector', function () {
        testScheduler.run(function (_a) {
            var hot = _a.hot, expectObservable = _a.expectObservable;
            var a = rxjs_1.throwError(function () { return new Error('too bad'); });
            var b = hot('---^--a--');
            var expected = '#';
            expectObservable(a.pipe(operators_1.buffer(b))).toBe(expected, null, new Error('too bad'));
        });
    ...
    it('should buffer when Promise resolves', function (done) {
        ...
        e1.pipe(operators_1.buffer(new Promise(function (resolve) { return setTimeout(function () { return resolve(); }, 8); }))).subscribe({
            next: function (x) {
                chai_1.expect(x).to.deep.equal(expected.shift());
            },
            error: function () { return done(new Error('should not be called')); },
            complete: function () {
    ...
    describe('equivalence with the window operator', function () {
        ...
        cases.forEach(function (_a, index) {
            var source = _a.source, notifier = _a.notifier;
            it("should be equivalent for case " + index, function () {
                testScheduler.run(function (_a) {
                    var hot = _a.hot, expectObservable = _a.expectObservable;
                    var a = hot(source);
                    var b = hot(notifier);
                    expectObservable(a.pipe(operators_1.buffer(b))).toEqual(a.pipe(operators_1.window(b), operators_1.mergeMap(function (w) { return w.pipe(operators_1.toArray()); })));
                });
            });
        });
    });
