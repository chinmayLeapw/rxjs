...
describe('map', function () {
    ...
    it('should map multiple values', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----------!';
            var expected = '--x--y--z--|';
            var result = e1.pipe(operators_1.map(function (x) { return 10 * +x; }));
    ...
    it('should map with index', function () {
        testScheduler.run(function (_a) {
            ...
            var invoked = 0;
            var result = e1.pipe(operators_1.map(function (x, index) {
                invoked++;
    ...
    it('should do multiple maps using a custom thisArg', function () {
        testScheduler.run(function (_a) {
            ...
            }, filterer), operators_1.map(function (x) {
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '--x--y-     ';
            var unsub = '   ------!     ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.map(addDrama), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected, { x: '1!', y: '2!' });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.map(rxjs_1.identity), operators_1.take(3)).subscribe(function () {
        });
