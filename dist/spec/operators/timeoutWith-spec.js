...
describe('timeoutWith operator', function () {
    ...
    it('should timeout after a specified period then subscribe to the passed observable', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var switchToSubs = ' -----^-----!  ';
            var expected = '     -----x-y-z-|  ';
            var result = source.pipe(operators_1.timeoutWith(t, switchTo, rxTestScheduler));
    ...
    it('should timeout at a specified date then subscribe to the passed observable', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var switchToSubs = ' ----------^----------!';
            var expected = '     ------------x--y--z--|';
            var result = source.pipe(operators_1.timeoutWith(new Date(t), switchTo, rxTestScheduler));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
            expectSubscriptions(switchTo.subscriptions).toBe(switchToSubs);
        });
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var expected = '    ---a---b----x--    ';
            var unsub = '       --------------!    ';
            var result = source.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.timeoutWith(t, switchTo, rxTestScheduler), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
            expectSubscriptions(switchTo.subscriptions).toBe(switchToSubs);
        });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.timeoutWith(0, rxjs_1.EMPTY), operators_1.take(3)).subscribe(function () {
        });
