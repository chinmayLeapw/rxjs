...
describe('elementAt', function () {
    ...
    it('should allow undefined as a default value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------------!   ';
            var expected = '--------------(U|)';
            expectObservable(e1.pipe(operators_1.elementAt(100, undefined))).toBe(expected, { U: undefined });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should raise error if e1 is Empty Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  (^!)';
            var expected = '#   ';
            expectObservable(e1.pipe(operators_1.elementAt(0))).toBe(expected, undefined, new rxjs_1.ArgumentOutOfRangeError());
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result Observable is unsubscribed', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-------     ';
            var unsub = '   ------!     ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.elementAt(2), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should raise error if index is out of range but does not have default value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----!';
            var expected = '-----#';
            expectObservable(e1.pipe(operators_1.elementAt(3))).toBe(expected, null, new rxjs_1.ArgumentOutOfRangeError());
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should return default value if index is out of range', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-----(x|)';
            var defaultValue = '42';
            expectObservable(e1.pipe(operators_1.elementAt(3, defaultValue))).toBe(expected, { x: defaultValue });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.elementAt(2)).subscribe(function () {
        });
