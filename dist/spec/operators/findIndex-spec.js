...
describe('findIndex', function () {
    ...
    it('should return matching element from source emits single element', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.findIndex(predicate))).toBe(expected, { x: 2 });
    ...
    it('should not emit if source does not emit', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^';
            var expected = '-';
            expectObservable(e1.pipe(operators_1.findIndex(truePredicate))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should return index of element from source emits single element', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.findIndex(predicate))).toBe(expected, { x: 0 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should return index of matching element from source emits multiple elements', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.findIndex(predicate))).toBe(expected, { x: 1 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should allow unsubscribing early and explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-------     ';
            var unsub = '   ------!     ';
            var result = e1.pipe(operators_1.findIndex(function (value) { return value === 'z'; }));
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-------     ';
            var unsub = '   ------!     ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.findIndex(function (value) { return value === 'z'; }), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should unsubscribe when the predicate is matched', function () {
        testScheduler.run(function (_a) {
            ...
            var t = time('    --|       ');
            var expected = '-------(x|) ';
            var result = e1.pipe(operators_1.findIndex(function (value) { return value === 'b'; }), operators_1.delay(t));
            expectObservable(result).toBe(expected, { x: 1 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should raise if source raise error while element does not match with predicate', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.findIndex(predicate))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.findIndex(function (value) { return value === 2; })).subscribe(function () {
        });
