...
describe('filter', function () {
    ...
    it('should filter in only prime numbers, source unsubscribes early', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '     --3---5----7-       ';
            var unsub = '        ------------!       ';
            expectObservable(e1.pipe(operators_1.filter(isPrime)), unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should invoke predicate once for each checked value', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.filter(predicate), operators_1.tap({
                complete: function () {
    ...
    it('should filter in only prime numbers, predicate with index, source unsubscribes early', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.filter(predicate)), unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should compose with another filter to allow multiples of six', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '       ^------------------!';
            var expected = '     --------6----------|';
            var result = e1.pipe(operators_1.filter(function (x) { return +x % 2 === 0; }), operators_1.filter(function (x) { return +x % 3 === 0; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should be able to accept and use a thisArg', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.filter(function (x) {
                return this.filter1(x);
            }, filterer), operators_1.filter(function (x) {
    ...
    it('should be able to use filter and map composed', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.filter(function (x) { return +x % 2 === 0; }), operators_1.map(function (x) { return +x * +x; }));
    ...
    it('should send errors down the error path', function (done) {
        rxjs_1.of(42)
            .pipe(operators_1.filter(function (x, index) {
            throw 'bad';
        }))
            .subscribe({
            next: function (x) {
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '     --3---5----7-       ';
            var unsub = '        ------------!       ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.filter(isPrime), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
    ...
    it('should support type guards without breaking previous behavior', function () {
        {
            ...
            var foo = new Foo();
            rxjs_1.of(foo)
                .pipe(operators_1.filter(function (foo) { return foo.baz === 42; }))
            ...
            var foobar = new Foo();
            rxjs_1.of(foobar)
                .pipe(operators_1.filter(function (foobar) { return foobar.bar === 'name'; }))
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(foobar)
                .pipe(operators_1.filter(isBar))
                .subscribe(function (x) { return x.bar; });
            ...
            rxjs_1.of(barish)
                .pipe(operators_1.filter(function (x) { return x.bar === 'quack'; }))
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(barish)
                .pipe(operators_1.filter(isBar))
        ...
        }
        {
            ...
            xs.pipe(operators_1.filter(isString)).subscribe(function (s) { return s.length; });
            xs.pipe(operators_1.filter(function (x) { return typeof x === 'number'; })).subscribe(function (x) { return x; });
            xs.pipe(operators_1.filter(function (x, i) { return typeof x === 'number' && x > i; })).subscribe(function (x) { return x; });
        }
    });
    it('should support Boolean as a predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '       ^------------------!';
            var expected = '     --t---t----t-------|';
            expectObservable(e1.pipe(operators_1.filter(Boolean))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.filter(function () { return true; }), operators_1.take(3))
            .subscribe(function () {
        });
