...
describe('observeOn', function () {
    ...
    it('should observe on specified scheduler', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------!';
            var expected = '--a--b--|';
            expectObservable(e1.pipe(operators_1.observeOn(testScheduler))).toBe(expected);
    ...
    it('should observe after specified delay', function () {
        testScheduler.run(function (_a) {
            ...
            var delay = time('  ---|       ');
            var expected = '  -----a----b-|';
            expectObservable(e1.pipe(operators_1.observeOn(testScheduler, delay))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when the result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '--a--    ';
            var unsub = '   ----!    ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.observeOn(testScheduler), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.observeOn(rxjs_1.queueScheduler), operators_1.take(3)).subscribe(function () {
        });
