...
describe('windowToggle', function () {
    ...
    it('should emit windows using varying hot closings', function () {
        rxTestScheduler.run(function (_a) {
            ...
            var values = { x: x, y: y, z: z };
            var i = 0;
            var result = e1.pipe(operators_1.windowToggle(e2, function () { return closings[i++]; }));
            expectObservable(result).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
    ...
    it('should dispose window Subjects if the outer is unsubscribed early', function () {
        rxTestScheduler.run(function (_a) {
            var hot = _a.hot, cold = _a.cold, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions, time = _a.time;
            var open = cold(' o-------------------------|');
            var e1 = hot('    --a--b--c--d--e--f--g--h--|');
            var e1subs = '    ^--------!                 ';
            var expected = '  x---------                 ';
            ...
            var values = { x: x };
            var window;
            var result = e1.pipe(operators_1.windowToggle(open, function () { return rxjs_1.NEVER; }), operators_1.tap(function (w) {
                window = w;
            }));
            expectObservable(result, unsub).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
