...
describe('skipUntil', function () {
    ...
    it('should allow unsubscribing explicitly and early', function () {
        testScheduler.run(function (_a) {
            ...
            var skipSubs = '  ^--------!          ';
            var expected = '  ----------          ';
            expectObservable(e1.pipe(operators_1.skipUntil(skip)), unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(skip.subscriptions).toBe(skipSubs);
        });
    });
    ...
    it('should stop listening to a synchronous notifier after its first nexted value', function () {
        var sideEffects = [];
        var synchronousNotifier = rxjs_1.concat(rxjs_1.defer(function () {
            sideEffects.push(1);
            return rxjs_1.of(1);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.skipUntil(rxjs_1.of(0)), operators_1.take(3)).subscribe(function () {
        });
    ...
    it('should skip until Promise resolves', function (done) {
        ...
        var expected = [2, 3, 4];
        e1.pipe(operators_1.skipUntil(new Promise(function (resolve) { return setTimeout(function () { return resolve(); }, 8); }))).subscribe({
            next: function (x) {
