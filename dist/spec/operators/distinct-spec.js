...
describe('distinct', function () {
    ...
    it('should distinguish values by key', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.distinct(selector))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should raises error when selector throws', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.distinct(selector))).toBe(expected, undefined, new Error('d is for dumb'));
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should support a flushing stream', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.distinct(undefined, e2))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should unsubscribe from the flushing stream when the main stream is unsubbed', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   -----------!         ';
            var expected = '--a--b------         ';
            expectObservable(e1.pipe(operators_1.distinct(undefined, e2)), unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
        });
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.distinct(), operators_1.take(3)).subscribe(function () {
        });
