...
describe('mergeMapTo', function () {
    ...
    it('should support the deprecated resultSelector', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.mergeMapTo(rxjs_1.of(4, 5, 6), function (a, b, i, ii) { return [a, b, i, ii]; }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should map values to constant resolved promises and merge', function (done) {
        ...
        var results = [];
        source.pipe(operators_1.mergeMapTo(rxjs_1.from(Promise.resolve(42)))).subscribe({
            next: function (x) {
                results.push(x);
            },
            error: function () {
    ...
    it('should map values to constant rejected promises and merge', function (done) {
        var source = rxjs_1.from([4, 3, 2, 1]);
        source.pipe(operators_1.mergeMapTo(rxjs_1.from(Promise.reject(42)))).subscribe({
            next: function () {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-----i---j---(ki)(lj)(ki)(lj)(ki)(lj)(ki)(lj)k---l---i-';
            var unsub = '   -------------------------------------------------------!';
            var result = e1.pipe(operators_1.map(function (x) { return x; }), operators_1.mergeMapTo(x), operators_1.map(function (x) { return x; }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.mergeMapTo(rxjs_1.of(0)), operators_1.take(3)).subscribe(function () {
        });
