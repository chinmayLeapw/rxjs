...
describe('count', function () {
    ...
    it('should count the values of an observable', function () {
        testScheduler.run(function (_a) {
            ...
            var subs = '      ^----------!';
            var expected = '  -----------(x|)';
            expectObservable(source.pipe(operators_1.count())).toBe(expected, { x: 3 });
    ...
    it('should work with error', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '   ^--------!';
            var expected = ' ---------#';
            expectObservable(e1.pipe(operators_1.count())).toBe(expected, null, 'too bad');
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should handle an always-true predicate on an empty hot observable', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.count(predicate))).toBe(expected, { w: 0 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should handle an always-true predicate on a simple hot observable', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.count(predicate))).toBe(expected, { w: 1 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should handle a match-all predicate on observable with many values', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.count(predicate))).toBe(expected, { w: 3 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
