...
describe('skipWhile', function () {
    ...
    it('should skip all elements with a true predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var sourceSubs = '   ^---------------!';
            var expected = '     ----------------|';
            var result = source.pipe(operators_1.skipWhile(function () { return true; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
        });
    });
    it('should skip all elements with a truthy predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var sourceSubs = '   ^---------------!';
            var expected = '     ----------------|';
            var result = source.pipe(operators_1.skipWhile(function () {
                return {};
            }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
    ...
    it('should not skip any element with a false predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var result = source.pipe(operators_1.skipWhile(function () { return false; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
        });
    });
    it('should not skip any elements with a falsy predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var sourceSubs = '   ^---------------!';
            var expected = '     -2--3--4--5--6--|';
            var result = source.pipe(operators_1.skipWhile(function () { return undefined; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var result = source.pipe(operators_1.mergeMap(function (x) {
                return rxjs_1.of(x);
            }), operators_1.skipWhile(predicate), operators_1.mergeMap(function (x) {
                return rxjs_1.of(x);
            }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(sourceSubs);
    ...
    it('should invoke predicate while its false and never again', function () {
        testScheduler.run(function (_a) {
            ...
            var result = source.pipe(operators_1.skipWhile(predicate), operators_1.tap({
                complete: function () {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.skipWhile(function (value) { return value < 2; }), operators_1.take(1))
            .subscribe(function () {
        });
