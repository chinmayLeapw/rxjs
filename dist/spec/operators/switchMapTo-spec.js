...
describe('switchMapTo', function () {
    ...
    it('should support the deprecated resultSelector', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.switchMapTo(rxjs_1.of(4, 5, 6), function (a, b, i, ii) { return [a, b, i, ii]; }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should unsub inner observables', function () {
        var unsubbed = 0;
        rxjs_1.of('a', 'b')
            .pipe(operators_1.switchMapTo(new rxjs_1.Observable(function (subscriber) {
            subscriber.complete();
            return function () {
                unsubbed++;
            };
    ...
    it('should switch to an inner cold observable, outer is unsubscribed early', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.switchMapTo(x)), unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should handle an empty outer', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  (^!)';
            var expected = '|   ';
            expectObservable(e1.pipe(operators_1.switchMapTo(rxjs_1.of('foo')))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.switchMapTo(rxjs_1.of(0)), operators_1.take(3)).subscribe(function () {
        });
