...
describe('mergeMap', function () {
    ...
    it('should map-and-flatten each item to an Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeMap(function (value) { return x.pipe(operators_1.map(function (i) { return i * +value; })); }));
    ...
    it('should support the deprecated resultSelector', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x, x + 1, x + 2); }, function (a, b, i, ii) { return [a, b, i, ii]; }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should support a void resultSelector (still deprecated)', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x, x + 1, x + 2); }, void 0))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should support a void resultSelector (still deprecated) and concurrency limit', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x, x + 1, x + 2); }, void 0, 1))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should map values to constant resolved promises and merge', function (done) {
        ...
        var project = function () { return rxjs_1.from(Promise.resolve(42)); };
        var results = [];
    ...
    it('should map values to constant rejected promises and merge', function (done) {
        var source = rxjs_1.from([4, 3, 2, 1]);
        var project = function () { return rxjs_1.from(Promise.reject(42)); };
        source.pipe(operators_1.mergeMap(project)).subscribe({
            next: function (x) {
    ...
    it('should mergeMap many outer values to many inner values', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.mergeMap(function () { return x; }));
    ...
    it('should mergeMap many outer to many inner, outer never completes', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   -------------------------------------------------------!';
            var source = e1.pipe(operators_1.mergeMap(function () { return x; }));
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-----i---j---(ki)(lj)(ki)(lj)(ki)(lj)(ki)(lj)k---l---i--';
            var unsub = '   -------------------------------------------------------!';
            var source = e1.pipe(operators_1.map(function (x) { return x; }), operators_1.mergeMap(function () { return x; }), operators_1.map(function (x) { return x; }));
            expectObservable(source, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    ...
    it('should mergeMap to many hot Observable, with parameter concurrency=1', function () {
        testScheduler.run(function (_a) {
            ...
            var a = hot('   x----i---j---k---l---|                                        ');
            var asubs = '   -^-------------------!                                        ';
            var b = hot('   -x-x-xxxx-x-x-xxxxx-x----i---j---k---l---|                    ');
            var bsubs = '   ---------------------^-------------------!                    ';
            var c = hot('   x-xxxx---x-x-x-x-x-xx--x--x-x--x--xxxx-x-----i---j---k---l---|');
            var csubs = '   -----------------------------------------^-------------------!';
            var e1 = hot('  -a-------b-------c---|                                        ');
            var e1subs = '  ^--------------------!                                        ';
            var expected = '-----i---j---k---l-------i---j---k---l-------i---j---k---l---|';
            var inners = { a: a, b: b, c: c };
            var project = function (x) { return inners[x]; };
            var result = e1.pipe(operators_1.mergeMap(project, 1));
            expectObservable(result).toBe(expected);
            expectSubscriptions(a.subscriptions).toBe(asubs);
            expectSubscriptions(b.subscriptions).toBe(bsubs);
    ...
    it('should support nested merges', function (done) {
        var results = [];
        rxjs_1.of(1)
            .pipe(operators_1.mergeMap(function () { return rxjs_1.defer(function () { return rxjs_1.of(2, rxjs_1.asapScheduler); }).pipe(operators_1.mergeMap(function () { return rxjs_1.defer(function () { return rxjs_1.of(3, rxjs_1.asapScheduler); }); })); }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should support nested merges with promises', function (done) {
        var results = [];
        rxjs_1.of(1)
            .pipe(operators_1.mergeMap(function () { return rxjs_1.from(Promise.resolve(2)).pipe(operators_1.mergeMap(function () { return Promise.resolve(3); })); }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should properly handle errors from iterables that are processed after some async', function () {
        testScheduler.run(function (_a) {
            ...
            var e1 = cold(' -----a------------b-----|', { a: ['o', 'o', 'o'], b: ['o', 'x', 'o'] });
            var e1subs = '  ^-----------------!      ';
            var expected = '-----(ooo)--------(o#)   ';
            ...
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x).pipe(operators_1.delay(0), operators_1.mergeMap(iterable)); }));
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.mergeMap(function (value) { return rxjs_1.of(value); }), operators_1.take(3))
            .subscribe(function () {
        });
