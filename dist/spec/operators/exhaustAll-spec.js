...
describe('exhaust', function () {
    ...
    it('should switch to first immediately-scheduled inner Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e2subs = [];
            var expected = '(ab|)';
            expectObservable(rxjs_1.of(e1, e2).pipe(operators_1.exhaustAll())).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   ----------------!            ';
            var expected = '--------a---b----            ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.exhaustAll(), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
    ...
    it('should handle an observable of promises, where one rejects', function (done) {
        rxjs_1.of(Promise.reject(2), Promise.resolve(1))
            .pipe(operators_1.exhaustAll())
            .subscribe({
            next: function (x) {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        rxjs_1.of(synchronousObservable)
    ...
    it('should handle synchronously completing inner observables', function (done) {
        var i = 1;
        rxjs_1.of(rxjs_1.of(1), rxjs_1.of(2))
            .pipe(operators_1.exhaustAll())
            .subscribe({
            next: function (v) { return chai_1.expect(v).to.equal(i++); },
            complete: function () {
