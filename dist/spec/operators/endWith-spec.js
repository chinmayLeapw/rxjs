...
describe('endWith', function () {
    ...
    it('should append numbers to a cold Observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-----------!   ';
            var expected = '---a--b--c--(s|)';
            expectObservable(e1.pipe(operators_1.endWith(values.s))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should end an observable with given value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----!   ';
            var expected = '--a--(x|)';
            expectObservable(e1.pipe(operators_1.endWith(defaultEndValue))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should raise error and not end with given value if source raises error', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-!';
            var expected = '--#';
            expectObservable(e1.pipe(operators_1.endWith(defaultEndValue))).toBe(expected, defaultEndValue);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '---a--b---        ';
            var unsub = '   ---------!        ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.endWith('s'), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should accept scheduler as last argument with single value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----!   ';
            var expected = '--a--(x|)';
            expectObservable(e1.pipe(operators_1.endWith(defaultEndValue, testScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should accept scheduler as last argument with multiple value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------!    ';
            var expected = '-----a--(yz|)';
            expectObservable(e1.pipe(operators_1.endWith('y', 'z', testScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.endWith(0), operators_1.take(3)).subscribe(function () {
        });
