...
describe('switchScan', function () {
    ...
    it('should map-and-flatten each item to an Observable while passing the accumulated value', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.switchScan(function (acc, x) { return e2.pipe(operators_1.map(function (i) { return i * Number(x) + acc; })); }, 0));
    ...
    it('should provide the proper accumulated values', function () {
        var accs = [];
        rxjs_1.of(1, 3, 5)
            .pipe(operators_1.switchScan(function (acc, x) {
            accs.push(acc);
    ...
    it('should unsub inner observables', function () {
        var unsubbed = [];
        rxjs_1.of('a', 'b')
            .pipe(operators_1.switchScan(function (_acc, x) {
    ...
    it('should switch inner cold observables', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.switchScan(function (_acc, value) { return observableLookup[value]; }, null));
    ...
    it('should raise error when projection throws', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.switchScan(project, null))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   ---------------------!                ';
            var observableLookup = { x: x, y: y };
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.switchScan(function (_acc, value) { return observableLookup[value]; }, null), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(x.subscriptions).toBe(xsubs);
            expectSubscriptions(y.subscriptions).toBe(ysubs);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        rxjs_1.of(null)
            .pipe(operators_1.switchScan(function () { return synchronousObservable; }, null), operators_1.takeWhile(function (x) { return x != 2; }))
            .subscribe(function () {
        });
    ...
    it('should handle outer empty', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  (^!)';
            var expected = '|   ';
            var result = e1.pipe(operators_1.switchScan(function (_acc, value) { return rxjs_1.of(value); }, ''));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should create a new seed for each subscriber', function () {
        var seeds = [];
        var observer = function (value) { return seeds.push(value); };
        var source = rxjs_1.of('a', 'b').pipe(operators_1.switchScan(function (acc, x) { return rxjs_1.of(acc + x); }, ''));
    ...
    it('should pass index to the accumulator function', function () {
        var indices = [];
        rxjs_1.of('a', 'b', 'c', 'd')
            .pipe(operators_1.switchScan(function (_acc, _x, index) {
            indices.push(index);
            return rxjs_1.of();
        }, ''))
            .subscribe();
