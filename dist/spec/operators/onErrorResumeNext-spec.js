...
describe('onErrorResumeNext', function () {
    ...
    it('should continue observable sequence with next observable', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.onErrorResumeNext(e2))).toBe(expected);
    ...
    it('should continue with multiple observables that throw errors', function () {
        testScheduler.run(function (_a) {
            ...
            var e4subs = '  ---------------------^-------!';
            var expected = '--a--b----c--d----e----f--g--|';
            expectObservable(e1.pipe(operators_1.onErrorResumeNext(e2, e3, e4))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        rxjs_1.throwError(function () { return new Error('Some error'); })
            .pipe(operators_1.onErrorResumeNext(synchronousObservable), operators_1.take(3))
            .subscribe(function () {
        });
    ...
    it('should unsubscribe from an interop observable upon explicit unsubscription', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   ------------!   ';
            var expected = '--a--b----c--   ';
            expectObservable(e1.pipe(operators_1.onErrorResumeNext(interop_helper_1.asInteropObservable(e2))), unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
        });
    });
    it('should work with promise', function (done) {
        var expected = [1, 2];
        var source = rxjs_1.concat(rxjs_1.of(1), rxjs_1.throwError(function () { return 'meh'; }));
    ...
    it('should skip invalid sources and move on', function () {
        var results = [];
        rxjs_1.of(1)
            .pipe(operators_1.onErrorResumeNext([2, 3, 4], { notValid: 'LOL' }, rxjs_1.of(5, 6)))
            .subscribe({
    ...
    it('should call finalize after each sync observable', function () {
        var results = [];
        rxjs_1.of(1)
            .pipe(operators_1.finalize(function () { return results.push('finalize 1'); }), operators_1.onErrorResumeNext(rxjs_1.of(2).pipe(operators_1.finalize(function () { return results.push('finalize 2'); })), rxjs_1.of(3).pipe(operators_1.finalize(function () { return results.push('finalize 3'); })), rxjs_1.of(4).pipe(operators_1.finalize(function () { return results.push('finalize 4'); }))))
            .subscribe({
            next: function (value) { return results.push(value); },
            complete: function () { return results.push('complete'); },
        });
        chai_1.expect(results).to.deep.equal([1, 'finalize 1', 2, 'finalize 2', 3, 'finalize 3', 4, 'finalize 4', 'complete']);
    });
    it('should not subscribe to the next source until after the previous is finalized.', function () {
        var results = [];
        rxjs_1.of(1)
            .pipe(operators_1.tap({
            subscribe: function () { return results.push('subscribe 1'); },
            finalize: function () { return results.push('finalize 1'); },
        }), operators_1.onErrorResumeNext(rxjs_1.of(2).pipe(operators_1.tap({
            subscribe: function () { return results.push('subscribe 2'); },
            finalize: function () { return results.push('finalize 2'); },
        })), rxjs_1.of(3).pipe(operators_1.tap({
            subscribe: function () { return results.push('subscribe 3'); },
            finalize: function () { return results.push('finalize 3'); },
        })), rxjs_1.of(4).pipe(operators_1.tap({
            subscribe: function () { return results.push('subscribe 4'); },
            finalize: function () { return results.push('finalize 4'); },
        }))))
            .subscribe({
            next: function (value) { return results.push(value); },
            complete: function () { return results.push('complete'); },
