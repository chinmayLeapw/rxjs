...
describe('concat operator', function () {
    ...
    it('should concatenate two cold observables', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.concat(e2, testScheduler))).toBe(expected);
        });
    });
    ...
    it('should accept scheduler with multiple observables', function () {
        testScheduler.run(function (_a) {
            ...
            var e3subs = '   ----------^-----!';
            var expected = ' ---a---b-----c--|';
            expectObservable(e1.pipe(operators_1.concat(e2, e3, testScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            expectSubscriptions(e2.subscriptions).toBe(e2subs);
    ...
    it('should accept scheduler without observable parameters', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '   ^----!';
            var expected = ' ---a-|';
            expectObservable(e1.pipe(operators_1.concat(testScheduler))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
