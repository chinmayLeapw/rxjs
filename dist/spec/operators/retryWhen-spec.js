...
describe('retryWhen', function () {
    ...
    it('should handle a source with eventual error using a hot notifier', function () {
        rxTest.run(function (_a) {
            ...
            var result = source.pipe(operators_1.retryWhen(function () { return notifier; }));
    ...
    it('should retry when notified via returned notifier on thrown error', function (done) {
        ...
        rxjs_1.of(1, 2, 3)
            ...
            return errors.pipe(operators_1.map(function (x) {
    ...
    it('should retry when notified and complete on returned completion', function (done) {
        ...
        rxjs_1.of(1, 2, 3)
            ...
            return n;
        }), operators_1.retryWhen(function () { return rxjs_1.EMPTY; }))
            .subscribe({
            next: function (n) {
    ...
    it('should propagate error thrown from notifierSelector function', function () {
        rxTest.run(function (_a) {
            ...
            var subs = '       ^----------!';
            var expected = '   --a--b--c--#';
            var result = source.pipe(operators_1.retryWhen(function () {
                throw 'bad!';
            }));
    ...
    it('should not break unsubscription chains when unsubscribed explicitly', function () {
        rxTest.run(function (_a) {
            ...
            var expected = '    -1--2-----1--2----1--       ';
            var unsub = '       --------------------!       ';
            var result = source.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.retryWhen(function () { return notifier; }), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(source.subscriptions).toBe(subs);
            expectSubscriptions(notifier.subscriptions).toBe(nsubs);
        });
    ...
    it('should handle a source with eventual error using a dynamic notifier selector which eventually throws', function () {
        rxTest.run(function (_a) {
            ...
            var result = source.pipe(operators_1.retryWhen(function (errors) {
                return errors.pipe(operators_1.map(function () {
                    if (++invoked === 3) {
                        throw 'error';
                    }
                    else {
    ...
    it('should handle a source with eventual error using a dynamic notifier selector which eventually completes', function () {
        rxTest.run(function (_a) {
            ...
            var invoked = 0;
            var result = source.pipe(operators_1.retryWhen(function (errors) {
                return errors.pipe(operators_1.map(function () { return 'x'; }), operators_1.takeUntil(errors.pipe(operators_1.mergeMap(function () {
                    if (++invoked < 3) {
                        return rxjs_1.EMPTY;
                    }
                    else {
    ...
    it('should always finalize before starting the next cycle, even when synchronous', function () {
        ...
        var subscription = source
            .pipe(operators_1.retryWhen(function (errors$) { return errors$.pipe(operators_1.mergeMap(function (err, i) { return (i < 3 ? rxjs_1.of(true) : rxjs_1.throwError(function () { return err; })); })); }))
            .subscribe({
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.retryWhen(function () { return rxjs_1.of(0); }), operators_1.take(3))
            .subscribe(function () {
        });
