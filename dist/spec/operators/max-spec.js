...
describe('max', function () {
    ...
    it('should allow unsubscribing explicitly and early', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-------     ';
            var unsub = '   ------!     ';
            expectObservable(e1.pipe(operators_1.max()), unsub).toBe(expected, { x: 42 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '-------     ';
            var unsub = '   ------!     ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.max(), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected, { x: 42 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should max a range() source observable', function (done) {
        rxjs_1.range(1, 10000)
    ...
    it('should work with error', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '   ^--------!';
            var expected = ' ---------#';
            expectObservable(e1.pipe(operators_1.max())).toBe(expected, null, 'too bad');
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should handle a constant predicate on a simple hot observable', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.max(predicate))).toBe(expected, { w: 1 });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should handle a predicate for string on observable with many values', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.max(predicate))).toBe(expected, { w: 'b' });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
