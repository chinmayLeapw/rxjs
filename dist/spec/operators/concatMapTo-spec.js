...
describe('concatMapTo', function () {
    ...
    it('should support the deprecated resultSelector', function () {
        var results = [];
        rxjs_1.of(1, 2, 3)
            .pipe(operators_1.concatMapTo(rxjs_1.of(4, 5, 6), function (a, b, i, ii) { return [a, b, i, ii]; }))
            .subscribe({
            next: function (value) {
                results.push(value);
            },
    ...
    it('should map values to constant resolved promises and concatenate', function (done) {
        ...
        var results = [];
        source.pipe(operators_1.concatMapTo(rxjs_1.from(Promise.resolve(42)))).subscribe({
            next: function (x) {
                results.push(x);
            },
            error: function (err) {
    ...
    it('should map values to constant rejected promises and concatenate', function (done) {
        var source = rxjs_1.from([4, 3, 2, 1]);
        source.pipe(operators_1.concatMapTo(rxjs_1.from(Promise.reject(42)))).subscribe({
            next: function (x) {
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.concatMapTo(rxjs_1.of(0)), operators_1.take(3)).subscribe(function () {
        });
