...
describe('debounce', function () {
    ...
    function getTimerSelector(x) {
        return function () { return rxjs_1.timer(x, testScheduler); };
    }
    ...
    it('should delay all element by selector observable', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.debounce(getTimerSelector(2)))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should support a scalar selector observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------------!';
            var expected = '--a--bc--d----|';
            expectObservable(e1.pipe(operators_1.debounce(function () { return rxjs_1.of(0); }))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '----a---       ';
            var unsub = '   -------!       ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.debounce(getTimerSelector(2)), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should debounce all elements while source emits by selector observable', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------------------!';
            var expected = '--------------------(e|)';
            expectObservable(e1.pipe(operators_1.debounce(getTimerSelector(4)))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should debounce all element while source emits by selector observable until raises error', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^------------!';
            var expected = '-------------#';
            expectObservable(e1.pipe(operators_1.debounce(getTimerSelector(5)))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should delay element by same selector observable emits multiple', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.debounce(function () { return selector; }))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should debounce by selector observable emits multiple', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.debounce(function () { return selector.shift(); }))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should raise error when selector function throws', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.debounce(selectorFunction))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
            for (var i = 0; i < selectorSubs.length; i++) {
                expectSubscriptions(selector[i].subscriptions).toBe(selectorSubs[i]);
            }
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.debounce(function () { return rxjs_1.of(0); }), operators_1.take(3))
            .subscribe(function () {
        });
