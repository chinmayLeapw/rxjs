...
describe('takeWhile', function () {
    ...
    it('should take all elements until predicate is false', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '   ^------!         ';
            var expected = ' -2--3--|         ';
            var result = e1.pipe(operators_1.takeWhile(function (v) { return +v < 4; }));
    ...
    it('should take all elements with truthy predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '    ^-------------!';
            var expected = '  --b--c--d--e--|';
            var result = e1.pipe(operators_1.takeWhile((function () {
                return {};
            })));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
    ...
    it('should skip all elements with predicate returns false', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '    ^-!            ';
            var expected = '  --|            ';
            var result = e1.pipe(operators_1.takeWhile(function () { return false; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should skip all elements with falsy predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '    ^-!            ';
            var expected = '  --|            ';
            var result = e1.pipe(operators_1.takeWhile(function () { return null; }));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should invoke predicate until return false', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.takeWhile(predicate), operators_1.tap({
                complete: function () {
    ...
    it('should support type guards without breaking previous behavior', function () {
        {
            ...
            var foo = new Foo();
            rxjs_1.of(foo)
                .pipe(operators_1.takeWhile(function (foo) { return foo.baz === 42; }))
            ...
            var foobar = new Foo();
            rxjs_1.of(foobar)
                .pipe(operators_1.takeWhile(function (foobar) { return foobar.bar === 'name'; }))
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(foobar)
                .pipe(operators_1.takeWhile(isBar))
                .subscribe(function (x) { return x.bar; });
            ...
            rxjs_1.of(barish)
                .pipe(operators_1.takeWhile(function (x) { return x.bar === 'quack'; }))
                .subscribe(function (x) { return x.bar; });
            rxjs_1.of(barish)
                .pipe(operators_1.takeWhile(isBar))
        ...
        }
        {
            ...
            xs.pipe(operators_1.takeWhile(isString)).subscribe(function (s) { return s.length; });
            xs.pipe(operators_1.takeWhile(function (x) { return typeof x === 'number'; })).subscribe(function (x) { return x; });
            xs.pipe(operators_1.takeWhile(function (x, i) { return typeof x === 'number' && x > i; })).subscribe(function (x) { return x; });
        }
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.takeWhile(function (value) { return value < 2; })).subscribe(function () {
        });
