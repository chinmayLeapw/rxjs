...
describe('throttle', function () {
    var testScheduler;
    beforeEach(function () {
        testScheduler = new TestScheduler_1.TestScheduler(observableMatcher_1.observableMatcher);
    });
    ...
    it('should handle sync source with sync notifier and trailing appropriately', function () {
        var results = [];
        var source = rxjs_1.of(1).pipe(operators_1.throttle(function () { return rxjs_1.of(1); }, { leading: false, trailing: true }));
        source.subscribe({
    ...
    it('should complete when source does not emit', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.throttle(durationSelector))).toBe(expected);
    ...
    describe('throttle(fn, { leading: true, trailing: true })', function () {
        it('should immediately emit the first value in each time window', function () {
            testScheduler.run(function (_a) {
                ...
                var result = e1.pipe(operators_1.throttle(function () { return e2; }, { trailing: true }));
                expectObservable(result).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
                expectSubscriptions(e2.subscriptions).toBe(e2subs);
            });
        ...
        it('should work for individual values', function () {
            testScheduler.run(function (_a) {
                ...
                var n1Subs = ['--^------------------!     '];
                var exp = '    --x------------------|     ';
                var result = s1.pipe(operators_1.throttle(function () { return n1; }, { trailing: true }));
    ...
    describe('throttle(fn, { leading: false, trailing: true })', function () {
        it('should immediately emit the first value in each time window', function () {
            testScheduler.run(function (_a) {
                ...
                var result = e1.pipe(operators_1.throttle(function () { return e2; }, { leading: false, trailing: true }));
                expectObservable(result).toBe(expected);
                expectSubscriptions(e1.subscriptions).toBe(e1subs);
                expectSubscriptions(e2.subscriptions).toBe(e2subs);
            });
        ...
        it('should work for individual values', function () {
            testScheduler.run(function (_a) {
                ...
                var n1Subs = ['--^-----------------------!   '];
                var exp = '    --------------------------(x|)';
                var result = s1.pipe(operators_1.throttle(function () { return n1; }, { leading: false, trailing: true }));
                expectObservable(result).toBe(exp);
                expectSubscriptions(s1.subscriptions).toBe(s1Subs);
                expectSubscriptions(n1.subscriptions).toBe(n1Subs);
            });
        ...
        it('should wait for trailing throttle before completing, even if source completes', function () {
            testScheduler.run(function (_a) {
                ...
                var durationSubs = ' ---^-----------------------!   ';
                var exp = '          ---------------------------(y|)';
                var result = source.pipe(operators_1.throttle(function () { return duration; }, { leading: false, trailing: true }));
                expectObservable(result).toBe(exp);
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable
            .pipe(operators_1.throttle(function () { return rxjs_1.of(0); }), operators_1.take(3))
            .subscribe(function () {
        });
