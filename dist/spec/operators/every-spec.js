...
describe('every', function () {
    ...
    it('should return false if only one of elements does not match with predicate', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----------!      ';
            var expected = '-----------(x|)   ';
            expectObservable(e1.pipe(operators_1.every(predicate))).toBe(expected, { x: false });
    ...
    it('should increment index on each call to the predicate', function () {
        var indices = [];
        rxjs_1.of(1, 2, 3, 4)
            .pipe(operators_1.every(function (_, i) {
            indices.push(i);
            return true;
        }))
            .subscribe();
    ...
    it('should emit true if source is empty', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----!   ';
            var expected = '-----(x|)';
            expectObservable(e1.pipe(operators_1.every(predicate))).toBe(expected, { x: true });
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chains when result observable is unsubscribed', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '--------          ';
            var unsub = '   -------!          ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.every(predicate), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should propagate error if predicate eventually throws', function () {
        testScheduler.run(function (_a) {
            ...
            expectObservable(e1.pipe(operators_1.every(faultyPredicate))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should raise error if source raises error', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-!';
            var expected = '--#';
            expectObservable(e1.pipe(operators_1.every(truePredicate))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
