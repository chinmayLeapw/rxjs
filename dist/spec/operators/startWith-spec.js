...
describe('startWith', function () {
    ...
    it('should not break unsubscription chains when result is unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var unsub = '   ---------!        ';
            var values = { s: 's', a: 'a', b: 'b' };
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.startWith('s', testScheduler), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
            expectObservable(result, unsub).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should accept scheduler as last argument with single value', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^----!';
            var expected = 'x-a--|';
            var result = e1.pipe(operators_1.startWith(defaultStartValue, testScheduler));
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.startWith(-1), operators_1.take(4)).subscribe(function () {
        });
