...
describe('dematerialize', function () {
    ...
    it('should dematerialize stream throws', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  (^!)';
            var expected = '#   ';
            expectObservable(e1.pipe(operators_1.dematerialize())).toBe(expected, null, error);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should work with materialize', function () {
        testScheduler.run(function (_a) {
            ...
            var result = e1.pipe(operators_1.materialize(), operators_1.dematerialize());
            expectObservable(result).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.materialize(), operators_1.dematerialize(), operators_1.take(3)).subscribe(function () {
        });
