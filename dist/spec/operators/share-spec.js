...
var syncError = rxjs_1.throwError(function () { return new Error(); });
...
describe('share', function () {
        ...
        describe(title, function () {
            ...
            it('should retry just fine', function () {
                rxTest.run(function (_a) {
                    ...
                    expectObservable(e1.pipe(operators_1.share(options), operators_1.retry(1))).toBe(expected);
                    expectSubscriptions(e1.subscriptions).toBe(e1subs);
                });
            });
            ...
            it('should share the same values to multiple observers, but is unsubscribed explicitly and early', function () {
                rxTest.run(function (_a) {
                    ...
                    var expected3 = '      ----------   ';
                    var shared = source.pipe(operators_1.share(options));
                    expectObservable(subscriber1.pipe(operators_1.mergeMapTo(shared)), unsub).toBe(expected1);
                    expectObservable(subscriber2.pipe(operators_1.mergeMapTo(shared)), unsub).toBe(expected2);
                    expectObservable(subscriber3.pipe(operators_1.mergeMapTo(shared)), unsub).toBe(expected3);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            });
            ...
            it('should disconnect when last subscriber unsubscribes', function () {
                rxTest.run(function (_a) {
                    ...
                    var expected2 = '      --------3----   ';
                    var shared = source.pipe(operators_1.share(options));
                    expectObservable(subscriber1.pipe(operators_1.mergeMapTo(shared)), unsub1).toBe(expected1);
                    expectObservable(subscriber2.pipe(operators_1.mergeMapTo(shared)), unsub2).toBe(expected2);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            });
            it('should not break unsubscription chain when last subscriber unsubscribes', function () {
                rxTest.run(function (_a) {
                    ...
                    var unsub2 = '         ------------!   ';
                    var expected2 = '      --------3----   ';
                    var shared = source.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.share(options), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
                    expectObservable(subscriber1.pipe(operators_1.mergeMapTo(shared)), unsub1).toBe(expected1);
                    expectObservable(subscriber2.pipe(operators_1.mergeMapTo(shared)), unsub2).toBe(expected2);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            ...
            it('should be repeatable when cold source is synchronous', function () {
                rxTest.run(function (_a) {
                    ...
                    var shared = source.pipe(operators_1.share(options));
                    expectObservable(hot(subscribe1).pipe(operators_1.tap(function () {
                        expectObservable(shared.pipe(operators_1.repeat(2))).toBe(expected1);
                    }))).toBe(subscribe1);
                    expectObservable(hot(subscribe2).pipe(operators_1.tap(function () {
                        expectObservable(shared.pipe(operators_1.repeat(2))).toBe(expected2);
                    }))).toBe(subscribe2);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            });
            it('should be retryable', function () {
                rxTest.run(function (_a) {
                    ...
                    var shared = source.pipe(operators_1.share(options));
                    expectObservable(hot(subscribe1).pipe(operators_1.tap(function () {
                        expectObservable(shared.pipe(operators_1.retry(2))).toBe(expected1);
                    }))).toBe(subscribe1);
                    expectObservable(hot(subscribe2).pipe(operators_1.tap(function () {
                        expectObservable(shared.pipe(operators_1.retry(2))).toBe(expected2);
                    }))).toBe(subscribe2);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            });
            it('should be repeatable', function () {
                rxTest.run(function (_a) {
                    ...
                    var shared = source.pipe(operators_1.share(options));
                    expectObservable(hot(subscribe1).pipe(operators_1.tap(function () {
                        expectObservable(shared.pipe(operators_1.repeat(3))).toBe(expected1);
                    }))).toBe(subscribe1);
                    expectObservable(hot(subscribe2).pipe(operators_1.tap(function () {
                        expectObservable(shared.pipe(operators_1.repeat(3))).toBe(expected2);
                    }))).toBe(subscribe2);
                    expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                });
            });
            ...
            it('should stop listening to a synchronous observable when unsubscribed', function () {
                ...
                synchronousObservable.pipe(operators_1.share(options), operators_1.take(3)).subscribe(function () {
                });
            ...
            it('should not fail on reentrant subscription', function () {
                rxTest.run(function (_a) {
                    ...
                    var subs = '       (^!)  ';
                    var expected = '   (136|)';
                    var deferred = rxjs_1.defer(function () { return shared; }).pipe(operators_1.startWith(0));
                    var shared = source.pipe(operators_1.withLatestFrom(deferred), operators_1.map(function (_a) {
    ...
    try {
        for (var _c = __values([
            { title: 'share()', options: {} },
            {
                ...
                options: {
                    resetOnError: function () { return syncNotify; },
                    resetOnComplete: function () { return syncNotify; },
                    resetOnRefCountZero: function () { return syncNotify; },
                },
            },
            {
                title: 'share() using sync reset notifiers equivalent to default config and notifying again after reset is notified',
                options: {
                    resetOnError: function () { return rxjs_1.concat(syncNotify, syncNotify); },
                    resetOnComplete: function () { return rxjs_1.concat(syncNotify, syncNotify); },
                    resetOnRefCountZero: function () { return rxjs_1.concat(syncNotify, syncNotify); },
                },
            },
            {
                title: 'share() using sync reset notifiers equivalent to default config and never completing after reset is notified',
                options: {
                    resetOnError: function () { return rxjs_1.concat(syncNotify, rxjs_1.NEVER); },
                    resetOnComplete: function () { return rxjs_1.concat(syncNotify, rxjs_1.NEVER); },
                    resetOnRefCountZero: function () { return rxjs_1.concat(syncNotify, rxjs_1.NEVER); },
                },
            },
            {
                title: 'share() using sync reset notifiers equivalent to default config and throwing an error after reset is notified',
                options: {
                    resetOnError: function () { return rxjs_1.concat(syncNotify, syncError); },
                    resetOnComplete: function () { return rxjs_1.concat(syncNotify, syncError); },
                    resetOnRefCountZero: function () { return rxjs_1.concat(syncNotify, syncError); },
                },
            },
        ]), _d = _c.next(); !_d.done; _d = _c.next()) {
            var _e = _d.value, title = _e.title, options = _e.options;
    ...
    var _loop_2 = function (title, resetOnError, resetOnComplete, resetOnRefCountZero) {
        describe(title, function () {
            it('should not reset on error if configured to do so', function () {
                rxTest.run(function (_a) {
                    ...
                    var result = source.pipe(operators_1.take(3), operators_1.share({ resetOnError: resetOnError }), operators_1.repeat());
            ...
            it('should not reset on complete if configured to do so', function () {
                rxTest.run(function (_a) {
                    ...
                    var count = 0;
                    var result = source.pipe(operators_1.takeUntil(triggerComplete), operators_1.share({ resetOnComplete: resetOnComplete }), operators_1.retry(), operators_1.tap(function () {
                        if (++count === 9) {
            ...
            it('should not reset on refCount 0 if configured to do so', function () {
                rxTest.run(function (_a) {
                    var hot = _a.hot, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions;
                    var source = hot('  ---v---v---v---E--v---v---v---C---v----v------v---');
                    var expected = '    ---v---v---v------v---v---v-------v----v----      ';
                    var subscription = '^-------------------------------------------!     ';
                    var sourceSubs = [
                        '                   ^--------------!',
                        '                   ---------------^--------------!',
                    ...
                    var result = source.pipe(operators_1.tap(function (value) {
                        if (value === 'E') {
                            throw new Error('E');
                        }
                    }), operators_1.takeWhile(function (value) { return value !== 'C'; }), operators_1.share({ resetOnRefCountZero: resetOnRefCountZero }), operators_1.retry(), operators_1.repeat());
            ...
            it('should be referentially-transparent', function () {
                rxTest.run(function (_a) {
                    ...
                    var partialPipeLine = rxjs_1.pipe(operators_1.share({ resetOnRefCountZero: resetOnRefCountZero }));
    ...
    try {
        for (var _f = __values([
            { title: 'share(config)', resetOnError: false, resetOnComplete: false, resetOnRefCountZero: false },
            {
                ...
                resetOnError: function () { return rxjs_1.EMPTY; },
                resetOnComplete: function () { return rxjs_1.EMPTY; },
                resetOnRefCountZero: function () { return rxjs_1.EMPTY; },
            },
            {
                title: 'share(config) using NEVER as sync reset notifier equivalents',
                resetOnError: function () { return rxjs_1.NEVER; },
                resetOnComplete: function () { return rxjs_1.NEVER; },
                resetOnRefCountZero: function () { return rxjs_1.NEVER; },
            },
        ]), _g = _f.next(); !_g.done; _g = _f.next()) {
            var _h = _g.value, title = _h.title, resetOnError = _h.resetOnError, resetOnComplete = _h.resetOnComplete, resetOnRefCountZero = _h.resetOnRefCountZero;
    ...
    describe('share(config)', function () {
        it('should use the connector function provided', function () {
            var connector = sinon_1.spy(function () { return new rxjs_1.Subject(); });
            rxTest.run(function (_a) {
                var hot = _a.hot, expectObservable = _a.expectObservable;
                var source = hot('  ---v---v---v---E--v---v---v---C---v----v--------v----v---');
                var subs1 = '       ^-------------------------------------------!            ';
                var expResult1 = '  ---v---v---v------v---v---v-------v----v-----            ';
                var subs2 = '       ----------------------------------------------^---------!';
                var expResult2 = '  ------------------------------------------------v----v---';
                var result = source.pipe(operators_1.tap(function (value) {
                    if (value === 'E') {
                ...
                }), operators_1.takeWhile(function (value) { return value !== 'C'; }), operators_1.share({
                    connector: connector,
                }), operators_1.retry(), operators_1.repeat());
    ...
    describe('share(config) with async/deferred reset notifiers', function () {
        it('should reset on refCount 0 when synchronously resubscribing to a firehose and using a sync reset notifier', function () {
            ...
            var result;
            source
                .pipe(operators_1.share({ resetOnRefCountZero: function () { return syncNotify; } }), operators_1.take(2), operators_1.repeat(2), operators_1.toArray())
                .subscribe(function (numbers) { return void (result = numbers); });
        ...
        it('should reset on refCount 0 when synchronously resubscribing and using a sync reset notifier', function () {
            rxTest.run(function (_a) {
                ...
                var subscription = '^--------------(- )        ';
                var sharedSource = source.pipe(operators_1.share({ resetOnRefCountZero: function () { return syncNotify; } }), operators_1.take(2));
        ...
        it('should not reset on refCount 0 when synchronously resubscribing and using a deferred reset notifier', function () {
            rxTest.run(function (_a) {
                ...
                var expected = '    ---1---2---3---4---5---|';
                var subscription = '^-----------------------';
                var sharedSource = source.pipe(operators_1.share({ resetOnRefCountZero: function () { return asapNotify; } }), operators_1.take(3));
                var result = rxjs_1.concat(sharedSource, sharedSource);
                expectObservable(result, subscription).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
            });
        ...
        it('should reset on refCount 0 only after reset notifier emitted', function () {
            rxTest.run(function (_a) {
                ...
                var sharedSource = source.pipe(operators_1.share({ resetOnRefCountZero: function () { return reset; } }), operators_1.take(2));
                var result = rxjs_1.concat(sharedSource, firstPause, sharedSource, secondPause, sharedSource);
                expectObservable(result, subscription).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
            });
        });
        it('should reset on error only after reset notifier emitted', function () {
            rxTest.run(function (_a) {
                ...
                var reset = cold('                 --r              ');
                var secondPause = cold('                     -----| ');
                var sharedSource = source.pipe(operators_1.share({ resetOnError: function () { return reset; }, resetOnRefCountZero: false }), operators_1.take(2));
                var result = rxjs_1.concat(sharedSource, firstPause, sharedSource, secondPause, sharedSource);
                expectObservable(result, subscription).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
            });
        ...
        it('should reset on complete only after reset notifier emitted', function () {
            rxTest.run(function (_a) {
                ...
                var reset = cold('                 --r              ');
                var secondPause = cold('                     -----| ');
                var sharedSource = source.pipe(operators_1.share({ resetOnComplete: function () { return reset; }, resetOnRefCountZero: false }), operators_1.take(2));
                var result = rxjs_1.concat(sharedSource, firstPause, sharedSource, secondPause, sharedSource);
                expectObservable(result, subscription).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
            });
        ...
        it('should not reset on refCount 0 if reset notifier errors before emitting any value', function () {
            spyOnUnhandledError(function (onUnhandledError) {
                ...
                chai_1.expect(onUnhandledError.getCall(0)).to.have.been.calledWithExactly(error);
                chai_1.expect(onUnhandledError.getCall(1)).to.have.been.calledWithExactly(error);
            });
        });
        ...
        it('should not call "resetOnRefCountZero" on error', function () {
            rxTest.run(function (_a) {
                var cold = _a.cold, expectObservable = _a.expectObservable, expectSubscriptions = _a.expectSubscriptions;
                var resetOnRefCountZero = sinon_1.spy(function () { return rxjs_1.EMPTY; });
                var source = cold('    ---1---(2#)                ');
                var sourceSubs = [
                    '                      ^------(! )                ',
                    '                      -------(- )---^------(! )  ',
                ...
                var firstPause = cold('       (- )---|            ');
                var reset = cold('            (- )-r              ');
                var sharedSource = source.pipe(operators_1.share({ resetOnError: function () { return reset; }, resetOnRefCountZero: resetOnRefCountZero }));
                var result = rxjs_1.concat(sharedSource.pipe(operators_1.onErrorResumeNext(firstPause)), sharedSource);
                expectObservable(result, subscription).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
        ...
        it('should not call "resetOnRefCountZero" on complete', function () {
            rxTest.run(function (_a) {
                ...
                var firstPause = cold('       (- )---|            ');
                var reset = cold('            (- )-r              ');
                var sharedSource = source.pipe(operators_1.share({ resetOnComplete: function () { return reset; }, resetOnRefCountZero: resetOnRefCountZero }));
                var result = rxjs_1.concat(sharedSource, firstPause, sharedSource);
                expectObservable(result, subscription).toBe(expected);
                expectSubscriptions(source.subscriptions).toBe(sourceSubs);
                chai_1.expect(resetOnRefCountZero).to.not.have.been.called;
