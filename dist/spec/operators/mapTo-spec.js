...
describe('mapTo', function () {
    ...
    it('should propagate errors from observable that emits only errors', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-!';
            var expected = '--#';
            expectObservable(e1.pipe(operators_1.mapTo(1))).toBe(expected, null, 'too bad');
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    it('should propagate errors from observable that emit values, then errors', function () {
        testScheduler.run(function (_a) {
            ...
            var e1subs = '  ^-------!';
            var expected = '--x--x--#';
            expectObservable(e1.pipe(operators_1.mapTo('x'))).toBe(expected, undefined, 'too bad');
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not break unsubscription chain when unsubscribed explicitly', function () {
        testScheduler.run(function (_a) {
            ...
            var expected = '--x--x-     ';
            var unsub = '   ------!     ';
            var result = e1.pipe(operators_1.mergeMap(function (x) { return rxjs_1.of(x); }), operators_1.mapTo('x'), operators_1.mergeMap(function (x) { return rxjs_1.of(x); }));
    ...
    it('should stop listening to a synchronous observable when unsubscribed', function () {
        ...
        synchronousObservable.pipe(operators_1.mapTo(0), operators_1.take(3)).subscribe(function () {
        });
