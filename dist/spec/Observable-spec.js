...
describe('Observable', function () {
    ...
    it('should send errors thrown in the constructor down the error path', function (done) {
        new rxjs_1.Observable(function () {
            ...
            error: function (err) {
                chai_1.expect(err).to.exist.and.be.instanceof(Error).and.have.property('message', 'this should be handled');
                done();
            },
        });
    });
    ...
    describe('forEach', function () {
        ...
        it('should reject promise when in error', function (done) {
            rxjs_1.throwError(function () { return ('bad'); })
                .forEach(function () {
        ...
        it('should allow Promise to be globally configured', function () { return __awaiter(void 0, void 0, void 0, function () {
            var wasCalled_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4, rxjs_1.of(42).forEach(function (x) {
    ...
    describe('subscribe', function () {
        ...
        it('should work with handlers with hacked bind methods, in the error case', function () {
            var source = rxjs_1.throwError(function () { return 'an error'; });
            var results = [];
        ...
        describe('when called with an anonymous observer', function () {
            ...
            it('should accept an anonymous observer with just an error function and call the error function in the context' +
                ...
                rxjs_1.throwError(function () { return ('bad'); }).subscribe(o);
            });
        ...
        describe('if config.useDeprecatedSynchronousErrorHandling === true', function () {
            ...
            it('should throw synchronously', function () {
                chai_1.expect(function () { return rxjs_1.throwError(function () { return new Error('thrown error'); }).subscribe(); }).to.throw(Error, 'thrown error');
            });
            ...
            it('should still rethrow synchronous errors from next handlers on synchronous observables', function () {
                chai_1.expect(function () {
                    rxjs_1.of('test').pipe(operators_1.map(function (x) { return x + '!!!'; }), operators_1.map(function (x) { return x + x; }), operators_1.map(function (x) { return x + x; }), operators_1.map(function (x) { return x + x; })).subscribe({
                        next: function () {
            ...
            it('should rethrow synchronous errors from flattened observables', function () {
                chai_1.expect(function () {
                    rxjs_1.of(1)
                        .pipe(operators_1.concatMap(function () { return rxjs_1.throwError(function () { return new Error('Ahoy! An error!'); }); }))
                        .subscribe(console.log);
                }).to.throw('Ahoy! An error!');
                chai_1.expect(function () {
                    rxjs_1.of(1)
                        .pipe(operators_1.switchMap(function () { return rxjs_1.throwError(function () { return new Error('Avast! Thar be a new error!'); }); }))
                        .subscribe(console.log);
            ...
            it('should not have a run-time error if no errors are thrown and there are operators', function () {
                chai_1.expect(function () {
                    rxjs_1.of(1, 2, 3).pipe(operators_1.map(function (x) { return x + x; }), operators_1.map(function (x) { return Math.log(x); }))
                        .subscribe();
                }).not.to.throw();
            });
            it('should call finalize if sync unsubscribed', function () {
                var called = false;
                var observable = new rxjs_1.Observable(function () { return function () { return (called = true); }; });
            ...
            it('should call registered finalizer if sync unsubscribed', function () {
                var called = false;
                var observable = new rxjs_1.Observable(function (subscriber) { return subscriber.add(function () { return called = true; }); });
                var subscription = observable.subscribe();
                subscription.unsubscribe();
                chai_1.expect(called).to.be.true;
            });
    ...
    it.skip('should handle sync errors within a test scheduler', function () {
        var observable = rxjs_1.of(4).pipe(operators_1.map(function (n) {
            if (n === 4) {
                throw 'four!';
            }
            return n;
        }), operators_1.catchError(function (err, source) { return source; }));
    ...
    it('should emit an error for unhandled synchronous exceptions from something like a stack overflow', function () {
        var source = new rxjs_1.Observable(function () {
            var boom = function () { return boom(); };
            boom();
        });
        var thrownError = undefined;
        source.subscribe({
            error: function (err) { return thrownError = err; }
        });
        chai_1.expect(thrownError).to.be.an.instanceOf(RangeError);
        chai_1.expect(thrownError.message).to.equal('Maximum call stack size exceeded');
    });
});
...
describe('Observable.lift', function () {
    ...
    it('should be overridable in a custom Observable type that composes', function (done) {
        ...
        result.subscribe({ next: function (x) {
                chai_1.expect(x).to.equal(expected.shift());
            }, error: function () {
                done(new Error('should not be called'));
            }, complete: function () {
    ...
    it('should compose through publish and refCount', function (done) {
        var result = new MyCustomObservable(function (observer) {
            ...
            observer.next(3);
            observer.complete();
        }).pipe(operators_1.publish(), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
        chai_1.expect(result instanceof MyCustomObservable).to.be.true;
        var expected = [10, 20, 30];
        result.subscribe({ next: function (x) {
                chai_1.expect(x).to.equal(expected.shift());
    ...
    it('should compose through publishLast and refCount', function (done) {
        var result = new MyCustomObservable(function (observer) {
            ...
            observer.next(3);
            observer.complete();
        }).pipe(operators_1.publishLast(), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
        chai_1.expect(result instanceof MyCustomObservable).to.be.true;
        var expected = [30];
        result.subscribe({ next: function (x) {
                chai_1.expect(x).to.equal(expected.shift());
    ...
    it('should compose through publishBehavior and refCount', function (done) {
        var result = new MyCustomObservable(function (observer) {
            ...
            observer.next(3);
            observer.complete();
        }).pipe(operators_1.publishBehavior(0), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
        chai_1.expect(result instanceof MyCustomObservable).to.be.true;
    ...
    it('should composes Subjects in the simple case', function () {
        ...
        var result = subject.pipe(operators_1.map(function (x) { return 10 * x; }));
        chai_1.expect(result instanceof rxjs_1.Subject).to.be.true;
        var emitted = [];
        result.subscribe(function (value) { return emitted.push(value); });
        result.next(10);
        result.next(20);
        result.next(30);
    ...
    it('should demonstrate the horrors of sharing and lifting the Subject through', function () {
        ...
        var result1 = shared.pipe(operators_1.map(function (x) { return x * 10; }));
        var result2 = shared.pipe(operators_1.map(function (x) { return x - 10; }));
        chai_1.expect(result1 instanceof rxjs_1.Subject).to.be.true;
        var emitted1 = [];
        result1.subscribe(function (value) { return emitted1.push(value); });
        var emitted2 = [];
        result2.subscribe(function (value) { return emitted2.push(value); });
        result1.next(10);
        result2.next(20);
        result1.next(30);
    ...
    describe.skip('The lift through Connectable gaff', function () {
        it('should compose through multicast and refCount, even if it is a Subject', function () {
            var subject = new rxjs_1.Subject();
            var result = subject.pipe(operators_1.multicast(function () { return new rxjs_1.Subject(); }), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
            chai_1.expect(result instanceof rxjs_1.Subject).to.be.true;
            var emitted = [];
            result.subscribe(function (value) { return emitted.push(value); });
            result.next(10);
        ...
        it('should compose through publish and refCount, even if it is a Subject', function () {
            var subject = new rxjs_1.Subject();
            var result = subject.pipe(operators_1.publish(), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
            chai_1.expect(result instanceof rxjs_1.Subject).to.be.true;
            var emitted = [];
            result.subscribe(function (value) { return emitted.push(value); });
            result.next(10);
        ...
        it('should compose through publishLast and refCount, even if it is a Subject', function () {
            var subject = new rxjs_1.Subject();
            var result = subject.pipe(operators_1.publishLast(), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
            chai_1.expect(result instanceof rxjs_1.Subject).to.be.true;
            var emitted = [];
            result.subscribe(function (value) { return emitted.push(value); });
            result.next(10);
        ...
        it('should compose through publishBehavior and refCount, even if it is a Subject', function () {
            var subject = new rxjs_1.Subject();
            var result = subject.pipe(operators_1.publishBehavior(0), operators_1.refCount(), operators_1.map(function (x) { return 10 * x; }));
            chai_1.expect(result instanceof rxjs_1.Subject).to.be.true;
            var emitted = [];
            result.subscribe(function (value) { return emitted.push(value); });
            result.next(10);
